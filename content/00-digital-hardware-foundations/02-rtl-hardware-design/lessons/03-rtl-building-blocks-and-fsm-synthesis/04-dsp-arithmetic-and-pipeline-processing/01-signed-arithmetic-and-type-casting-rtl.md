content/00-digital-hardware-foundations/02-rtl-hardware-design/lessons/03-rtl-building-blocks-and-fsm-synthesis/04-dsp-arithmetic-and-pipeline-processing/01-signed-arithmetic-and-type-casting-rtl.md
...
## The Sub-Zero Temperature Thermometer and the Inventory Counter: An Everyday Mental Model

To build an intuitive mental model of signedness propagation, zero-extension vs. sign-extension, and arithmetic right shifts before examining SystemVerilog syntax, let us picture two different measuring devices used in an industrial warehouse.

### Part A: The Sub-Zero Temperature Sensor (Signed Two's Complement)

Imagine a cold-storage warehouse in Alaska equipped with a digital temperature sensor ($A$). The sensor measures temperature in degrees Celsius using an 8-bit digital readout screen.
* Above zero ($+05^\circ\text{C}$): Displayed as positive binary numbers.
* Below zero ($-05^\circ\text{C}$): Displayed as negative Two's Complement binary numbers.

When the temperature is $-05^\circ\text{C}$, the sensor is measuring a true **signed quantity**. If you drop the temperature by another $2^\circ\text{C}$, the mathematical equation computes $-5 - 2 = -7^\circ\text{C}$.
...
### The Contamination Disaster (Mixing Signed and Unsigned)

Suppose a computer system reads values from both sensors and computes a combined warehouse metric using a single equation:

$$\text{Combined Metric} = \text{Temperature Sensor } A + \text{Apple Counter } B$$

Suppose the temperature drops to $-05^\circ\text{C}$ (`8'b1111_1011` in 8-bit Two's Complement), and the apple counter reads $02$ crates (`8'b0000_0010`).

```text
TEMPERATURE SENSOR VS APPLE COUNTER CONTAMINATION

 Temp Sensor A (-5 C)  :  1 1 1 1  1 0 1 1  (Signed Two's Complement = -5)
 Apple Counter B (2)   :  0 0 0 0  0 0 1 0  (Unsigned Item Counter = +2)
                          ─────────────────
 If Computer treats expression as UNSIGNED:
 Temp Sensor A is read as +251 crates!
 Equation computes: 251 + 2 = 253!  (CORRUPTED RESULT!)
```

Look at what happens if the computer does not know that Sensor $A$ is signed:
* The computer reads the bit pattern `8'b1111_1011` from Sensor $A$.
* Because Apple Counter $B$ is unsigned, the computer assumes the entire equation is an apple-counting problem!
* The computer interprets `8'b1111_1011` as **251 crates of apples**!
* It calculates $251 + 2 = 253$ crates of apples!

Now, suppose the computer writes this result into a 16-bit master ledger. Because the computer thought it was counting apples (unsigned), it pads the top 8 bits with **zeros ($0000\_0000_2$)**:

$$\text{16-Bit Ledger Value} = 0000\_0000\_1111\_1101_2 = +253_{10}$$

Instead of recording $-3^\circ\text{C}$ (`16'b1111_1111_1111_1101`), the computer recorded **$+253$**! A freezing temperature of $-3^\circ\text{C}$ was recorded as a scorching $+253^\circ\text{C}$!
...
## Mechanics of Two's Complement Signed Declarations and Type Casting

To master signed arithmetic in SystemVerilog, we must examine the formal mechanics of vector signedness declarations and type casting.
...
### Primitive 2: SystemVerilog Expression Signedness Propagation Rules

When a SystemVerilog compiler evaluates an expression containing multiple variables, parameters, or constants (such as `Y = A + B * C`), it determines the signedness of the calculation according to four strict IEEE 1800 rules:

#### Rule 1: The All-Signed Rule
An expression is evaluated as **signed** IF AND ONLY IF **every single operand in the expression is signed**.

$$\text{Expression Signedness} = \text{Signed} \quad \iff \quad \forall \text{ operands } X_i, \, \text{Signed}(X_i) = \text{True}$$

#### Rule 2: The Infectious Unsigned Contamination Rule
If **even ONE operand** in an expression is unsigned, **the entire expression is evaluated as unsigned**. Every signed operand in the expression is silently converted to unsigned before the operation takes place!

```systemverilog
logic signed [7:0] a; // Signed (-128 to +127)
logic        [7:0] b; // Unsigned (0 to 255)
logic signed [15:0] result;

// INFECTIOUS UNSIGNED CONTAMINATION
// Because 'b' is unsigned, 'a' is converted to unsigned!
// Addition is performed as UNSIGNED!
assign result = a + b; 
```

#### Rule 3: Concatenation Strips Signedness
The vector concatenation operator `{a, b}` **ALWAYS yields an UNSIGNED result**, regardless of whether `a` and `b` were declared `signed`.

```systemverilog
logic signed [7:0] a, b;
logic signed [15:0] result;

// CONCATENATION STRIPS SIGNEDNESS!
// {a, b} is treated as an UNSIGNED 16-bit vector!
assign result = {a, b} + 1; 
```

#### Rule 4: Vector Slices Are Unsigned
Any bit-slice of a vector (such as `signed_vec[3:0]`) **ALWAYS yields an UNSIGNED result**, even if `signed_vec` was declared `signed`.

```systemverilog
logic signed [15:0] audio_word;
logic signed [15:0] result;

// VECTOR SLICE STRIPS SIGNEDNESS!
// audio_word[7:0] is treated as UNSIGNED!
assign result = audio_word[7:0] + 1; 
```
...
## Shift Operators: Logical (`>>`, `<<`) versus Arithmetic (`>>>`, `<<<`)

One of the most frequent operations in digital signal processing is scaling a data vector by powers of two ($2^K$).

SystemVerilog provides four distinct shift operators, divided into two categories:
1. **Logical Shift Operators**: `>>` (Logical Shift Right) and `<<` (Logical Shift Left).
2. **Arithmetic Shift Operators**: `>>>` (Arithmetic Shift Right) and `<<<` (Arithmetic Shift Left).
...
SHIFT OPERATOR CLASSIFICATION
  ...
  (Always Fill 0s)                      (Types Dictate Fill)
...
## Engineering Reality: Real-World Pitfalls in Signed SystemVerilog RTL

In commercial semiconductor design, signedness errors are among the hardest bugs to detect because the code compiles without errors and passes simple positive-number testbenches, only to fail when negative numbers enter the pipeline.
...
### Pitfall 2: Intermediate Multiplier Width Truncation

When multiplying two $N$-bit signed numbers, the resulting product can span up to $2N$ bits.

Consider multiplying two 8-bit signed numbers ($A \times B$):

```systemverilog
logic signed [7:0]  coeff_a;  // -8
logic signed [7:0]  sample_b; // +5
logic signed [15:0] product;

// CORRECT 16-BIT SIGNED MULTIPLIER SYNTHESIS
assign product = coeff_a * sample_b; // Synthesizes an 8x8 signed multiplier (16-bit result)
```

Now, suppose `sample_b` was declared as an **unsigned** 8-bit vector (`logic [7:0] sample_b`):

```systemverilog
logic signed [7:0]  coeff_a;  // -8 (Signed)
logic        [7:0]  sample_b; // +5 (UNSIGNED!)
logic signed [15:0] product;

// CONTAMINATION BUG!
// Because sample_b is unsigned, coeff_a is converted to UNSIGNED!
// Synthesizes an 8x8 UNSIGNED multiplier!
assign product = coeff_a * sample_b; 
```

#### What the Hardware Compiler Does:
Because `sample_b` is unsigned, the compiler converts `coeff_a` ($-8_{10} = 8\text{'b}1111\_1000$) to unsigned $+248_{10}$! 

The synthesis tool builds an **8-bit x 8-bit Unsigned Multiplier**:
$$\text{Product} = 248 \times 5 = 1240_{10} = 16\text{'b}0000\_0100\_1101\_1000$$

Instead of computing $-8 \times 5 = -40_{10}$ (`16'b1111_1111_1101_1000`), the hardware computes **$+1240_{10}$**!

#### The Fix:
Cast `sample_b` explicitly using `$signed({1'b0, sample_b})`:

```systemverilog
// CORRECT MIXED SIGNED MULTIPLICATION
// Zero-extend sample_b by 1 bit, then cast to signed!
assign product = coeff_a * $signed({1'b0, sample_b}); // Correct -40 result!
```
...
## Solved Industrial Engineering Exercise: Multi-Sensor Environmental Temperature & Calibration Processor

To consolidate your complete mastery of signed vectors, `$signed()` type casting, arithmetic right shifts (`>>>`), mixed-operand expression propagation, and sign-extension, we will now walk through a complete, step-by-step digital engineering problem.
...
We construct `SensorDataProcessor` using clean, explicit SystemVerilog signed arithmetic constructs:

```systemverilog
`default_nettype none

// MULTI-SENSOR ENVIRONMENTAL TEMPERATURE & CALIBRATION PROCESSOR
module SensorDataProcessor (
    input  logic signed [7:0]  raw_temp,           // 8-bit signed temperature (-128 to +127)
    input  logic        [7:0]  cal_offset,         // 8-bit unsigned calibration offset (0 to 255)
    input  logic        [1:0]  shift_scale,        // 2-bit scaling shift count (0 to 3)
    output logic signed [15:0] processed_temp_out, // 16-bit signed scaled output
    output logic signed [15:0] mult_product_out    // 16-bit signed product output
);

    // Internal 9-bit signed intermediate sum
    // Sign-extended raw_temp (9 bits) + zero-extended cal_offset cast to signed (9 bits)
    logic signed [8:0] sum_raw;
    logic signed [8:0] sum_scaled;

    always_comb begin
        // Step 1: Signed Addition with Explicit Casting
        // $signed({1'b0, cal_offset}) zero-extends cal_offset by 1 bit and casts to signed!
        sum_raw = $signed({raw_temp[7], raw_temp}) + $signed({1'b0, cal_offset});

        // Step 2: Arithmetic Right Shift
        // Operates on signed sum_raw, performing true sign-extended right shift!
        sum_scaled = sum_raw >>> shift_scale;

        // Step 3: Explicit Sign Extension to 16 Bits
        // Assigns 9-bit signed to 16-bit signed target (automatic sign extension)
        processed_temp_out = $signed(sum_scaled);

        // Step 4: Mixed-Sign Multiplication
        // Multiply 8-bit signed raw_temp by 8-bit unsigned cal_offset safely!
        mult_product_out = raw_temp * $signed({1'b0, cal_offset});
    end
...
#### Step 3: Trace Test Case 2 (Negative Temperature)

* Inputs:
  * `raw_temp = -40` (`8'b1101_1000` = `8'hD8`).
  * `cal_offset = 10` (`8'b0000_1010` = `8'h0A`).
  * `shift_scale = 2` (`2'b10`, Shift Right by 2 positions).

##### Step-by-Step Execution Trace:
1. **Step 1 (Signed Addition)**:
   * `raw_temp` sign-extended to 9 bits: `9'sb1_1101_1000` ($-40_{10}$).
   * `cal_offset` zero-extended to 9 bits and cast to signed: `$signed(9'b0_0000_1010)` ($+10_{10}$).
   * `sum_raw = -40 + 10 = -30_{10}` (`9'sb1_1110_0010`).
   * Verify Two's Complement: $-256 + 128 + 64 + 32 + 2 = -30_{10}$.
2. **Step 2 (Arithmetic Shift Right `>>> 2`)**:
   * `sum_raw = 9'sb1_1110_0010`. Sign bit = $1$.
   * Shift right by 2 positions with sign-fill ($11_2$):
     $$\text{sum\_scaled} = 9\text{'sb}1\_1111\_1000 = -8_{10}$$
   * Verify Two's Complement: $-256 + 128 + 64 + 32 + 16 + 8 = -8_{10}$.
3. **Step 3 (Sign Extension to 16 Bits)**:
   * `processed_temp_out = 16'sb1111_1111_1111_1000` ($-8_{10}$ = `16'hFFF8`).

```text
TEST CASE 2 EXECUTION TRACE (NEGATIVE SIGN PRESERVED!)

 raw_temp (-40)           : 1 _ 1 1 0 1 _ 1 0 0 0  (9-Bit Signed -40)
 $signed({1'b0, cal})     : 0 _ 0 0 0 0 _ 1 0 1 0  (9-Bit Signed +10)
                            ─────────────────────
 sum_raw (-30)            : 1 _ 1 1 1 0 _ 0 0 1 0  (9-Bit Signed -30)
 sum_scaled (>>> 2)       : 1 _ 1 1 1 1 _ 1 0 0 0  (9-Bit Signed -8)
 processed_temp_out (16b) : 16'hFFF8 (-8 Decimal!)
```

##### Mathematical Check:
$$\left\lfloor \frac{-40 + 10}{2^2} \right\rfloor = \left\lfloor \frac{-30}{4} \right\rfloor = \lfloor -7.5 \rfloor = \mathbf{-8_{10}}$$
**ARITHMETIC SHIFT PRESERVED SIGN AND EXECUTED CORRECT FLOOR DIVISION!**
...
* **Signed Type Casting (`$signed()`)**: The SystemVerilog explicit casting operator that forces an expression, vector slice, or concatenated array to be evaluated as a Two's Complement signed quantity, overriding the default infectious unsigned propagation rule and enabling correct sign-extension during bit-width expansion.
* **Infectious Unsigned Expression Propagation**: The SystemVerilog expression evaluation rule where the presence of even a single unsigned operand converts all other operands in the expression tree to unsigned, causing negative Two's Complement values to be zero-extended into large positive numbers.
* **Arithmetic Shift Right (`>>>`)**: The signed shift operator that performs signed division by powers of two ($V / 2^K$) on `logic signed` vectors by filling vacated Most Significant Bit positions with copies of the Two's Complement Sign Bit ($b_{N-1}$).
* **Explicit Sign Extension**: The zero-hardware wiring pattern (`{ {(M-N){val[N-1]}}, val }`) that expands an $N$-bit signed vector to an $M$-bit signed target by replicating the sign bit $b_{N-1}$ across the top $M-N$ bit positions.
