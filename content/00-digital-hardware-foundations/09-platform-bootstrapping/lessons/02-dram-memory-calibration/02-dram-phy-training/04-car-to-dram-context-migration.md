content/00-digital-hardware-foundations/09-platform-bootstrapping/lessons/02-dram-memory-calibration/02-dram-phy-training/04-car-to-dram-context-migration.md
...
## 2. Changing Tires on a Moving Race Car

To build an intuitive, crystal-clear mental model of Cache-as-RAM teardown, stack frame pointer relocation, and the atomic transition from temporary SRAM to physical DRAM before inspecting assembly instruction sequences, MTRR bitfield writes, and memory delta equations, let us consider an everyday analogy: **Changing Tires on a Moving Formula 1 Race Car**.

Imagine a high-speed Formula 1 race car (**The CPU Execution Core**) driving down the main straightaway of a racetrack at 200 miles per hour (**Executing Active Boot Firmware Code**).

```text
THE MOVING RACE CAR TIRE SWAP ANALOGY

 Race Car at 200 MPH (CPU Core)               Pit Crew (Migration Engine)
 ┌───────────────────────────┐                ┌───────────────────────────┐
 │ Driving on Temporary      │                │ Prepares New Rubber Tires │
 │ Wooden Training Tires     │                │ On Mobile Parallel Rig    │
 └─────────────┬─────────────┘                └─────────────▲─────────────┘
               │                                            │
               ▼ (Tire Swap executed WHILE DRIVING!)        │
 ┌──────────────────────────────────────────────────────────┴─────────────┐
 │ 1. Build parallel platform next to car (Prepare DRAM Stack)            │
 │ 2. Pivot car weight onto rubber tires (Atomic SP Pointer Swap)         │
 │ 3. Unbolt wooden tires (WBINVD / Teardown CAR Mode)                    │
 └────────────────────────────────────────────────────────────────────────┘
```

The car is currently driving on temporary, fragile wooden training tires (**Temporary Cache-as-RAM Stack**). The wooden tires were installed at the start of the race because the main paved track was covered in ice (**Un-configured System DRAM**).

Now, the ice has melted, and the track is paved, dry, and ready (**Physical DRAM Initialized and Calibrated**). The pit crew has laid four brand-new, ultra-high-grip rubber tires (**Physical DRAM Memory**) next to the pit wall.

The race director enforces an extreme rule: *"The race car MUST NOT STOP or drop below 200 miles per hour! You must swap the car's wheels from the temporary wooden training tires to the permanent rubber tires WHILE THE CAR IS DRIVING AT FULL SPEED!"*

Look at what happens under different tire-swapping attempts:

* **Attempt A (Un-bolting the Wooden Tires First)**:
  If a pit mechanic unbolts the wooden tires while the car is moving at 200 MPH before the new rubber tires are attached:
  * The car drops onto its bare metal underbody, sparks fly, the chassis shatters, and the car crashes into the wall (**Executing `INVD` $\to$ Active Stack Erased & CPU Crashes**)!
* **Attempt B (The Synchronized Parallel Swap)**:
  To swap the wheels safely at 200 MPH without crashing, the pit crew executes a **3-Step Synchronized Pivot Protocol**:

1. **Build a Mobile Parallel Platform (Allocate DRAM Stack Buffer)**: The pit crew drives a flatbed support vehicle alongside the race car at the exact same speed ($200\text{ MPH}$). They mount the new rubber tires onto the flatbed rig right next to the car's wheel hubs (**Copy Active Stack Bytes from CAR to Physical DRAM**).
2. **The Atomic Weight Pivot (Modify Stack Pointer Register `SP`)**: In one single, synchronized movement, a mechanical hydraulic arm reaches out, locks onto the new rubber tire axles, and **pivots the car's physical weight off the wooden tires and onto the new rubber tires** (**Atomic Stack Pointer Update: `SP = SP + Delta`**)!
   * The car is now rolling $100\%$ on the new rubber tires!
3. **Unbolt and Recycles the Old Wooden Tires (CAR Teardown)**: Now that the car's weight is supported safely by the rubber tires, mechanics unbolt the old wooden tires and throw them into the recycling bin (**Execute `WBINVD` & Reset MTRRs**).

The car continues racing down the track at 200 MPH without losing a single millisecond of speed or dropping a single millimeter toward the asphalt!

This synchronized moving tire swap is the exact physical analogue of **Cache-as-RAM Teardown and Stack Migration Execution**:
* The race car driving at 200 MPH is the **Active CPU Execution Core running compiled C code**.
* Temporary wooden training tires are the **Temporary Cache-as-RAM (CAR) Stack**.
* Permanent high-grip rubber tires are **Trained Physical DRAM Memory**.
* Unbolting tires while driving is **Invalidating CAR cache lines with `INVD`**.
* The parallel flatbed rig is the **Destination DRAM Stack Buffer**.
* Pivoting the car's weight is **Atomically Updating the Stack Pointer Register (`SP`)**.
* Throwing away wooden tires is **Executing `WBINVD` and Re-programming MTRRs**.
...
### Primitive 1: The Stack Migration Mathematical Offset Equation

Let $A_{\text{car\_base}}$ be the physical base address of the temporary Cache-as-RAM memory window (for example, `0xFEF0_0000`).

Let $A_{\text{dram\_base}}$ be the physical base address of the new, permanent stack memory region allocated inside calibrated DRAM (for example, `0x0010_0000`).

The **Migration Delta ($\Delta_{\text{migration}}$)** is the signed mathematical offset between the old CAR stack base and the new DRAM stack base:

$$\mathbf{\Delta_{\text{migration}} = A_{\text{dram\_base}} - A_{\text{car\_base}}}$$

Where:
* $\Delta_{\text{migration}}$ is the signed 64-bit address offset added to convert any CAR address into its corresponding DRAM address.
* $A_{\text{dram\_base}}$ is the 64-bit physical start address of the DRAM stack region.
* $A_{\text{car\_base}}$ is the 64-bit physical start address of the CAR SRAM window.

If the active Stack Pointer register (`SP`) currently points to address $A_{\text{sp\_car}}$ inside the CAR window, the target Stack Pointer address $A_{\text{sp\_dram}}$ in physical DRAM is:

$$\mathbf{A_{\text{sp\_dram}} = A_{\text{sp\_car}} + \Delta_{\text{migration}} = A_{\text{sp\_car}} + (A_{\text{dram\_base}} - A_{\text{car\_base}})}$$
...
### Primitive 2: Cache-as-RAM Teardown Sequence

Now that the CPU's Stack Pointer (`RSP`) and Frame Pointer (`RBP`) are running safely on physical DRAM memory, the temporary Cache-as-RAM environment must be completely dismantled.

```text
CAR TEARDOWN STATE MACHINE

 Stack Running on DRAM Memory
               │
               ▼
 Step 1: Disable Caching (Set CR0.CD = 1)
               │
               ▼
 Step 2: Write-Back and Invalidate Caches (Execute WBINVD)
         (Flushes any remaining dirty CAR lines to DRAM & clears tags!)
               │
               ▼
 Step 3: Clear Temporary CAR MTRRs (Reset MTRR 0 Base & Mask)
               │
               ▼
 Step 4: Program Permanent DRAM MTRRs (Set 0..RAM_Size to Write-Back 0x06)
               │
               ▼
 Step 5: Re-enable Caching (Clear CR0.CD = 0)
               │
               ▼
 L1/L2 Caches Restored to Standard Transparent DRAM Caching Mode!
```
...
## 4. Engineering Realities: Dangling Pointers and Secondary Core (AP) CAR Teardown

In commercial platform engineering, executing stack migration and CAR teardown introduces critical physical edge cases that can trigger silent memory corruption if software conventions are violated.
...
### 2. Secondary Core (AP) CAR Teardown in Multi-Core Sockets

On multi-core processors, each physical CPU core contains its own **private L1 Data Cache**.

During early boot, when the Bootstrap Processor (BSP / Core 0) configures CAR mode, **CAR mode is active ONLY inside Core 0's private L1 cache**!

When secondary Application Processor (AP) cores are awakened later in the boot sequence:
1. Each AP core powers up with its own private L1 cache in an uninitialized state.
2. If an AP core attempts to access the old CAR address `0xFEF0_0000`, its local L1 cache will experience a miss and dispatch an un-coherent bus request!
3. **Multi-Core Rule**: Each secondary AP core must execute its own local cache invalidation (`INVD`) and MTRR synchronization before participating in multi-core execution, ensuring that all cores share a unified view of physical DRAM!
...
You are a principal platform software architect verifying the CAR-to-DRAM stack migration sequence for a $3.2\text{-GHz}$ 64-bit server processor core ($T_{\text{clk}} = 0.3125\text{ ns} = 312.5\text{ ps}$).

The processor is currently executing early C boot code inside a temporary Cache-as-RAM (CAR) window:

$$\text{CAR Window Base Address } (A_{\text{car\_base}}) = \text{0x0000\_0000\_FEF0\_0000}$$

$$\text{CAR Window Top Address } (A_{\text{car\_top}}) = \text{0x0000\_0000\_FEFF\_FFFF} \quad (1\text{ MB Window Size} = 1,048,576\text{ Bytes})$$

Physical DRAM memory has been trained and validated. The operating system driver allocates a new permanent stack region in physical DRAM:

$$\text{DRAM Stack Base Address } (A_{\text{dram\_base}}) = \text{0x0000\_0000\_0010\_0000}$$

$$\text{DRAM Stack Top Address } (A_{\text{dram\_top}}) = \text{0x0000\_0000\_001F\_FFFF} \quad (1\text{ MB Window Size} = 1,048,576\text{ Bytes})$$

```text
STACK MIGRATION HARDWARE PARAMETERS

 Parameter Symbol          │ Value                 │ Description
───────────────────────────┼───────────────────────┼─────────────────────────────────────────────
 f_cpu                     │ 3.2 GHz (3,200 MHz)   │ Core CPU execution clock frequency
 RSP_car                   │ 0x0000_0000_FEFF_8000 │ Current Stack Pointer value in CAR
 N_saved_frame_pointers    │ 16 Saved Frames       │ Number of saved RBP pointers on active stack
 BW_dram_copy              │ 30.72 GB / Second     │ On-chip L1-to-DRAM bus memory copy speed
 Cycles_fp_fixup           │ 12 Clock Cycles / Frame Time to relocate 1 saved frame pointer
 Cycles_wbinvd             │ 2,048 Clock Cycles    │ WBINVD instruction execution delay
 Cycles_mtrr_write         │ 512 Clock Cycles      │ MTRR re-programming MSR write overhead
```
...
#### Step 1: Calculate Active Stack Payload Size ($\text{Stack\_Size}$)

The stack grows downward from $A_{\text{car\_top}} + 1 = \text{0x0000\_0000\_FF00\_0000}$.

Current Stack Pointer $\text{RSP}_{\text{car}} = \text{0x0000\_0000\_FEFF\_8000}$.

$$\text{Stack\_Size} = (A_{\text{car\_top}} + 1) - \text{RSP}_{\text{car}}$$

$$\text{Stack\_Size} = \text{0x0000\_0000\_FF00\_0000} - \text{0x0000\_0000\_FEFF\_8000}$$

$$\text{Stack\_Size} = \text{0x0000\_0000\_0000\_8000} = \mathbf{32,768 \text{ Bytes}} \quad (32\text{ KB})$$

The active C call stack currently occupies **$32,768\text{ bytes}$ ($32\text{ KB}$)** of memory.
...
#### Step 3: Calculate Stack Memory Copy Latency ($t_{\text{memcpy}}$)

Copying the $32,768\text{-byte}$ stack payload from CAR SRAM to physical DRAM at a memory copy bandwidth of $30.72\text{ GB/sec}$ ($30,720,000,000\text{ bytes/sec}$):

$$t_{\text{memcpy}} = \frac{\text{Stack\_Size}}{\text{BW}_{\text{dram\_copy}}} = \frac{32,768\text{ Bytes}}{30,720,000,000\text{ Bytes/sec}}$$

$$t_{\text{memcpy}} = 1.066667 \times 10^{-6}\text{ seconds} = \mathbf{1,066.667 \text{ Nanoseconds}} \quad (1.0667\ \mu\text{s})$$

In CPU clock cycles ($T_{\text{clk}} = 0.3125\text{ ns}$):

$$C_{\text{memcpy}} = \frac{1,066.667\text{ ns}}{0.3125\text{ ns/cycle}} = \mathbf{3,413.33 \approx 3,414 \text{ CPU Clock Cycles}}$$
...
#### Step 5: Calculate Total CAR Teardown & Migration Latency ($T_{\text{teardown\_total}}$)

The complete end-to-end teardown and migration sequence consumes:

$$C_{\text{total}} = C_{\text{memcpy}} + C_{\text{fp\_fixup}} + C_{\text{sp\_swap}} + C_{\text{wbinvd}} + C_{\text{mtrr\_write}}$$

Assuming atomic SP swap instruction overhead $C_{\text{sp\_swap}} = 4\text{ cycles}$:

$$C_{\text{total}} = 3,414 + 192 + 4 + 2,048 + 512 = \mathbf{6,170 \text{ CPU Clock Cycles}}$$

Calculate total physical execution time $T_{\text{teardown\_total}}$ at $3.2\text{ GHz}$:

$$T_{\text{teardown\_total}} = 6,170 \text{ cycles} \times 0.3125\text{ ns/cycle} = \mathbf{1,928.125 \text{ Nanoseconds}} = \mathbf{1.928125 \text{ Microseconds}}$$

```text
STACK MIGRATION AND CAR TEARDOWN TIMELINE SUMMARY

 Execution Phase               │ CPU Clock Cycles (3.2 GHz) │ Physical Latency
───────────────────────────────┼────────────────────────────┼─────────────────────────
 Active Stack Copy (32 KB)     │ 3,414 Cycles               │ 1,066.875 Nanoseconds
 Frame Pointer Fixup (16 RBP)  │   192 Cycles               │    60.000 Nanoseconds
 Atomic SP Register Swap       │     4 Cycles               │     1.250 Nanoseconds
 WBINVD Cache Flush            │ 2,048 Cycles               │   640.000 Nanoseconds
 MTRR System Re-programming    │   512 Cycles               │   160.000 Nanoseconds
───────────────────────────────┼────────────────────────────┼─────────────────────────
 TOTAL CAR TEARDOWN & MIGRATION│ 6,170 Cycles               │ 1.928125 Microseconds
```

##### Engineering Conclusion:
In **$1.928125\text{ microseconds}$ ($6,170\text{ CPU clock cycles}$)**, the platform firmware migrated its active 32-KB C execution stack from temporary CAR SRAM to physical DRAM, updated its stack pointers, flushed dirty lines, re-programmed MTRRs, and restored normal L1/L2 caching mode **without losing a single return address or dropping a single execution cycle**!
...
* **Stack Migration Execution**: The low-level assembly protocol where an active C call stack payload is copied from temporary Cache-as-RAM (CAR) SRAM to physical DRAM, saved frame pointers (`RBP`) are relocated by adding a signed Migration Delta ($\Delta_{\text{migration}} = A_{\text{dram\_base}} - A_{\text{car\_base}}$), and the Stack Pointer register (`SP`) is atomically updated without corrupting active function return paths.
* **Cache-as-RAM Teardown**: The hardware re-configuration sequence where early boot firmware executes a Write-Back Invalidate (`WBINVD`) instruction to commit dirty CAR lines to DRAM, clears temporary CAR MTRRs, programs permanent DRAM Write-Back memory types, and clears $CR0.CD$ to restore L1/L2/L3 caches back to standard transparent DRAM caching mode.