content/00-digital-hardware-foundations/04-memory-subsystems/lessons/04-dram-architecture-controllers/03-memory-controller-scheduling/01-fr-fcfs-command-scheduling.md
...
## The Bank Teller and the Document Binders: An Everyday Mental Model

To build an intuitive, crystal-clear mental model of FR-FCFS command scheduling, out-of-order reordering, and starvation prevention before analyzing formal scheduling algorithms, state matrix logic, and timing equations, let us consider an everyday analogy: **The Bank Teller and the Archival Document Binders**.

Imagine a busy bank teller (**The Memory Controller Command Scheduler**) servicing a line of waiting customers (**The Memory Command Queue**).

```text
THE BANK TELLER AND DOCUMENT BINDERS METAPHOR

 Line of Waiting Customers (Command Queue)       Central Bank Vault
 ┌───────────────────────────┐                   ┌───────────────────────────┐
 │ Customer 1: Binder A      │                   │ Heavy Document Binders    │
 │ Customer 2: Binder B      │                   │ Retrieval Time: 14 Mins   │
 │ Customer 3: Binder A      │                   └───────────────────────────┘
 │ Customer 4: Binder A      │                     (Main System DRAM Memory)
 └───────────────────────────┘
```

In the basement vault sits a collection of heavy document binders (**DRAM Rows**). Inside each binder are 1,000 individual paper forms (**Column Words**).

On the teller's desk sits a small reading tray (**The Row Buffer**). The reading tray is small: it can hold **only one document binder (8 Kilobytes)** at a time.

Retrieving a binder from the basement vault, opening it, and setting it on the reading tray takes **14 minutes** ($t_{\text{RCD}}$ Row Activation). Reading a single page off an open binder on the desk takes **1 minute** ($t_{\text{CL}}$ Column Read). Putting a binder away and locking the vault takes **14 minutes** ($t_{\text{RP}}$ Row Precharge).

Let us observe two different management rules for how the bank teller services the customer line:
...
### Strategy 2: The Smart Reordering Rule (First-Ready First-Come First-Served / FR-FCFS)

The bank manager fires the rigid teller and hires an intelligent teller who uses the **FR-FCFS Rule**:

$$\text{"Rule 1: If a customer wants a page from a binder ALREADY OPEN on the desk, CALL THEM FIRST!"}$$
$$\text{"Rule 2: Otherwise, serve the customer who has been waiting in line the longest."}$$

Now, trace how the intelligent teller services the exact same line:

```text
SMART REORDERING WORKFLOW (FR-FCFS STRATEGY)

 1. Fetch Binder A (14m) ──► Open Binder A on Desk
 2. Call Customer 1 (Binder A) ──► Read Page in 1 Min! (Row Buffer Hit)
 3. Call Customer 3 (Binder A) ──► Read Page in 1 Min! (Row Buffer Hit - REORDERED!)
 4. Call Customer 4 (Binder A) ──► Read Page in 1 Min! (Row Buffer Hit - REORDERED!)
 5. Put Binder A away (14m) ──► Fetch Binder B (14m)
 6. Call Customer 2 (Binder B) ──► Read Page in 1 Min!
 (Total Time = 47 Minutes! Saved 55 Minutes! 54% Faster!)
```

Look at what Strategy 2 achieved:
1. **Binder A stayed open on the desk!** The teller called Customer 1, Customer 3, and Customer 4 back-to-back because all three wanted pages from Binder A!
2. **Zero Redundant Vault Trips**: Binder A was fetched from the basement **ONCE** instead of three times!
3. **Massive Time Savings**: Total service time dropped from $102\text{ minutes}$ down to **$47\text{ minutes}$**—a **$54\%$ throughput gain**!
...
## Primitive 1: First-Ready First-Come First-Served (FR-FCFS) Scheduling Logic

Now that we possess a clear intuitive mental model of the smart bank teller, let us examine the formal, rigorous engineering mechanics of **First-Ready First-Come First-Served (FR-FCFS) Command Scheduling** (pioneered by Scott Rixner et al. in 2000).

> **FR-FCFS (First-Ready First-Come First-Served)** is an out-of-order memory command scheduling policy implemented inside hardware memory controllers that prioritizes memory requests targeting **currently open Row Buffers** (First-Ready) over requests targeting closed or conflicting rows, falling back to arrival order (First-Come First-Served) among requests with equal readiness status.

```text
FR-FCFS TWO-TIER PRIORITY DECISION MATRIX

 Memory Requests Sitting in Controller Command Queue
                       │
       Is any request READY (Row Buffer Hit)?
                       │
             ┌─────────┴─────────┐
             │ YES               │ NO (All are Row Misses/Conflicts)
             ▼                   ▼
     PRIORITIZE READY!     FALL BACK TO AGE!
     Select oldest Row     Select oldest request
     Buffer Hit request.   in queue (FCFS rule).
     (Rule 1: First-Ready) (Rule 2: First-Come)
```
...
### Mathematical Proof of FR-FCFS Latency Reduction

Let us mathematically compare the bus latency required to execute $N_{\text{hits}}$ consecutive requests targeting the same open row under FCFS versus FR-FCFS.

Let $T_{\text{activate}} = t_{\text{RCD}} \approx 14\text{ ns}$ be the Row-to-Column Activate Delay.
Let $T_{\text{precharge}} = t_{\text{RP}} \approx 14\text{ ns}$ be the Row Precharge Delay.
Let $T_{\text{read}} = t_{\text{CL}} \approx 10\text{ ns}$ be the Column Read Latency.

#### Under Naive In-Order FCFS (With 1 Interleaved Conflict Request):
If $N_{\text{hits}}$ requests to Row 10 are interrupted by 1 single conflict request to Row 20 in the middle:

$$T_{\text{FCFS}} = (T_{\text{activate}} + T_{\text{read}}) + \Big( N_{\text{hits}} \times (T_{\text{precharge}} + T_{\text{activate}} + T_{\text{read}}) \Big)$$

Every interrupted request pays a full $T_{\text{precharge}} + T_{\text{activate}}$ conflict penalty ($28\text{ ns}$)!

#### Under Out-of-Order FR-FCFS:
FR-FCFS reorders all $N_{\text{hits}}$ requests to Row 10 together *first*, and then services the conflict request to Row 20 *last*:

$$T_{\text{FR-FCFS}} = T_{\text{activate}} + (N_{\text{hits}} \times T_{\text{read}}) + T_{\text{precharge}} + T_{\text{activate}} + T_{\text{read}}$$

$$\mathbf{\text{Latency Saved} = (N_{\text{hits}} - 1) \times (T_{\text{precharge}} + T_{\text{activate}})}$$

Where:
* $\text{Latency Saved}$ is the total physical execution time saved by FR-FCFS in nanoseconds.
* $N_{\text{hits}}$ is the number of reordered row buffer hit requests.
* $T_{\text{precharge}} + T_{\text{activate}}$ is the combined row conflict setup overhead ($t_{\text{RP}} + t_{\text{RCD}} \approx 28\text{ ns}$).

If $N_{\text{hits}} = 4$ requests, FR-FCFS saves $(4 - 1) \times 28\text{ ns} = \mathbf{84 \text{ nanoseconds}}$ ($268\text{ CPU clock cycles}$)!
...
### The Parallel FR-FCFS Priority Evaluator Tree

On every memory bus clock cycle, the memory controller's scheduler evaluates the priority of all $M$ slots simultaneously using a parallel **Combinational Priority Encoder Tree**:

```text
PARALLEL FR-FCFS PRIORITY ENCODER TREE

 Entry 0 : Ready=1, Age=105 ──┐
 Entry 1 : Ready=0, Age=400 ──┼──► [ Priority Evaluator Tree ] ──► Select Entry 0!
 Entry 2 : Ready=1, Age=12  ──┤    (Ready=1 beats Ready=0!)        (Dispatches Read)
 Entry 3 : Ready=0, Age=50  ──┘
```

The priority score $P_k$ for slot $k$ is calculated as:

$$P_k = (\text{Ready}_k \cdot 2^{16}) + \text{Age}_k$$

Where:
* $P_k$ is the composite integer priority score for slot $k$.
* $\text{Ready}_k$ is the 1-bit readiness flag ($1$ for Row Buffer Hit, $0$ otherwise).
* $2^{16}$ is a large weighting multiplier ($65,536$) ensuring that any Ready request ($1 \cdot 65,536$) scores higher than any non-ready request regardless of age.
* $\text{Age}_k$ is the current age counter value in clock cycles.

The priority encoder selects the slot with the highest composite score $P_k$ and dispatches its command across the memory bus!
...
### 1. Request Starvation and Anti-Starvation Aging Thresholds ($A_{\text{max}}$)

#### The Starvation Hazard:
Suppose a multi-core CPU executes a streaming thread on Core 0 that continuously reads sequential elements from an $8\text{-KB}$ row in Bank 0 (generating thousands of consecutive Row Buffer Hits).

Simultaneously, Core 1 dispatches a single memory read targeting a *different* row in Bank 0 (a Row Buffer Conflict).

Under pure FR-FCFS:
* As long as Core 0 keeps generating Row Buffer Hits, Rule 1 ($\text{Ready} = 1$) continuously prioritizes Core 0's requests over Core 1's request.
* Core 1's request sits frozen in the command queue for millions of cycles!
* Core 1 suffers **Starvation**, freezing Core 1's execution pipeline indefinitely!

```text
THE FR-FCFS STARVATION HAZARD

 Streaming Core 0 (Row 10 Hits) ──► [ Ready = 1 ] ──┐
 Streaming Core 0 (Row 10 Hits) ──► [ Ready = 1 ] ──┼──► Selected continuously!
 Streaming Core 0 (Row 10 Hits) ──► [ Ready = 1 ] ──┘
                                                         Core 1 Frozen Forever!
 Stalled Core 1   (Row 20 Conf) ──► [ Ready = 0 ] ──────► (STARVATION!)
```

#### The Hardware Fix: Anti-Starvation Aging Threshold ($A_{\text{max}}$)
To guarantee forward progress for all threads, the memory controller enforces an **Anti-Starvation Aging Threshold ($A_{\text{max}}$)** (typically $A_{\text{max}} = 256 \text{ to } 1,024\text{ clock cycles}$):

$$\text{If } \text{Age}_k \ge A_{\text{max}} \implies \text{Override Priority: } \text{Set Ready}_k \Leftarrow 1 \quad (\mathbf{\text{FORCE SERVICING!}})$$

```text
ANTI-STARVATION AGING THRESHOLD OVERRIDE

 Core 1 Request Age reaches A_max (512 Cycles)!
                            │
                            ▼
 Hardware Override: Force Core 1 Priority Above All Row Hits!
 Close Row 10 ──► Open Row 20 ──► Service Core 1 Request!
 (Starvation broken! Core 1 un-stalls!)
```

When Core 1's request age reaches $A_{\text{max}}$, the hardware **overrides the First-Ready rule**, forces Bank 0 to close Row 10, opens Row 20, and services Core 1. Fairness is restored!
...
## Solved Industrial Engineering Exercise: Quantitative FR-FCFS Reordering Trace, Bandwidth Gain, and Anti-Starvation Analysis

To consolidate your complete mastery of FR-FCFS scheduling logic, out-of-order command queue reordering, row buffer hit mechanics, and anti-starvation aging thresholds, we will now walk through a complete, step-by-step industrial hardware engineering problem.
...
#### Step 1: Analyze Naive In-Order FCFS Execution (Strict Arrival Order)

In-order FCFS executes requests in strict arrival order: **Req 1 $\to$ Req 2 $\to$ Req 3 $\to$ Req 4 $\to$ Req 5 $\to$ Req 6**.

##### 1. Req 1 (`Bank 0, Row 20, Col 0` — Conflict with Open Row 10!):
* Bank 0 has Row 10 open. Must precharge Row 10, activate Row 20, read Col 0.
* Cycle 0: Issue `PRECHARGE Bank 0` ($t_{\text{RP}} = 14\text{ cycles}$).
* Cycle 14: Issue `ACTIVATE Bank 0, Row 20` ($t_{\text{RCD}} = 14\text{ cycles}$).
* Cycle 28: Issue `READ Bank 0, Col 0` ($t_{\text{CL}} = 14\text{ cycles}$). Data arrives at **Cycle 42** ($26.25\text{ ns}$).

##### 2. Req 2 (`Bank 0, Row 10, Col 8` — Conflict with Open Row 20!):
* Bank 0 now has Row 20 open! Req 2 targets Row 10 $\implies$ Conflict!
* Cycle 29: Issue `PRECHARGE Bank 0` ($t_{\text{RP}} = 14\text{ cycles}$).
* Cycle 43: Issue `ACTIVATE Bank 0, Row 10` ($t_{\text{RCD}} = 14\text{ cycles}$).
* Cycle 57: Issue `READ Bank 0, Col 8` ($t_{\text{CL}} = 14\text{ cycles}$). Data arrives at **Cycle 71** ($44.375\text{ ns}$).

##### 3. Req 3 (`Bank 1, Row 50, Col 0` — Closed Bank Miss):
* Bank 1 is closed.
* Cycle 58: Issue `ACTIVATE Bank 1, Row 50` ($t_{\text{RCD}} = 14\text{ cycles}$).
* Cycle 72: Issue `READ Bank 1, Col 0` ($t_{\text{CL}} = 14\text{ cycles}$). Data arrives at **Cycle 86** ($53.75\text{ ns}$).

##### 4. Req 4 (`Bank 0, Row 10, Col 16` — Row Buffer Hit on Row 10):
* Bank 0 currently has Row 10 open!
* Cycle 73: Issue `READ Bank 0, Col 16` ($t_{\text{CL}} = 14\text{ cycles}$). Data arrives at **Cycle 87** ($54.375\text{ ns}$).

##### 5. Req 5 (`Bank 0, Row 20, Col 8` — Conflict with Open Row 10!):
* Cycle 87: Issue `PRECHARGE Bank 0` ($t_{\text{RP}} = 14\text{ cycles}$).
* Cycle 101: Issue `ACTIVATE Bank 0, Row 20` ($t_{\text{RCD}} = 14\text{ cycles}$).
* Cycle 115: Issue `READ Bank 0, Col 8` ($t_{\text{CL}} = 14\text{ cycles}$). Data arrives at **Cycle 129** ($80.625\text{ ns}$).

##### 6. Req 6 (`Bank 0, Row 10, Col 24` — Conflict with Open Row 20!):
* Cycle 130: Issue `PRECHARGE Bank 0` ($t_{\text{RP}} = 14\text{ cycles}$).
* Cycle 144: Issue `ACTIVATE Bank 0, Row 10` ($t_{\text{RCD}} = 14\text{ cycles}$).
* Cycle 158: Issue `READ Bank 0, Col 24` ($t_{\text{CL}} = 14\text{ cycles}$). Data arrives at **Bus Cycle 172** ($107.50\text{ ns}$).

```text
IN-ORDER FCFS TOTAL EXECUTION TIME
Total Bus Cycles = 172 t_CK
Total Execution Time = 172 x 0.625 ns = 107.50 NANOSECONDS (344 CPU Cycles)
```
...
##### FR-FCFS Execution Sequence:

##### 1. Req 2 (`Bank 0, Row 10, Col 8` — Reordered Row Hit!):
* Row 10 is ALREADY open in Bank 0!
* Bus Cycle 0: Issue **`READ Bank 0, Col 8`** ($t_{\text{CL}} = 14\text{ cycles}$). Data arrives at **Cycle 14** ($8.75\text{ ns}$).

##### 2. Req 4 (`Bank 0, Row 10, Col 16` — Reordered Row Hit!):
* Row 10 is STILL open in Bank 0!
* Bus Cycle 1: Issue **`READ Bank 0, Col 16`** ($t_{\text{CL}} = 14\text{ cycles}$). Data arrives at **Cycle 15** ($9.375\text{ ns}$).

##### 3. Req 6 (`Bank 0, Row 10, Col 24` — Reordered Row Hit!):
* Row 10 is STILL open in Bank 0!
* Bus Cycle 2: Issue **`READ Bank 0, Col 24`** ($t_{\text{CL}} = 14\text{ cycles}$). Data arrives at **Cycle 16** ($10.00\text{ ns}$).

##### All Row 10 Hits Completed by Bus Cycle 16 ($10.0\text{ ns}$)!
...
Let us compare total execution time for the 6 requests:

##### 1. Total Execution Time (System 0 - In-Order FCFS):
$$T_{\text{FCFS}} = 172 \text{ bus cycles} \times 0.625 \text{ ns/cycle} = \mathbf{107.50 \text{ nanoseconds}} \quad (344\text{ CPU Clock Cycles})$$

##### 2. Total Execution Time (System 1 - FR-FCFS Reordered):
$$T_{\text{FR-FCFS}} = 47 \text{ bus cycles} \times 0.625 \text{ ns/cycle} = \mathbf{29.375 \text{ nanoseconds}} \quad (94\text{ CPU Clock Cycles})$$

##### 3. Calculate Performance Speedup Factor:

$$\text{Speedup} = \frac{T_{\text{FCFS}}}{T_{\text{FR-FCFS}}} = \frac{107.50\text{ ns}}{29.375\text{ ns}} = \frac{172\text{ bus cycles}}{47\text{ bus cycles}} \approx \mathbf{3.6596\times \text{ Performance Advantage!}}$$

```text
FR-FCFS SCHEDULING PERFORMANCE COMPARISON

 Scheduling Algorithm │ Total Bus Cycles │ Total Time (ns) │ CPU Stall Cycles │ Speedup Factor
──────────────────────┼──────────────────┼─────────────────┼──────────────────┼────────────────
 In-Order FCFS (FIFO) │ 172 Cycles       │ 107.50 ns       │ 344 Cycles       │ 1.00x (Base)
 FR-FCFS Out-of-Order │  47 Cycles       │  29.38 ns       │  94 Cycles       │ 3.66x FASTER!
                      │ (72.7% Saved!)   │ (78.12 ns Saved)│ (250 Cycles Saved)│ (+266% Gain)
```

FR-FCFS out-of-order scheduling made memory execution **$3.66\times$ faster ($266\%$ throughput gain)**!
...
Let us verify our mathematical and hardware scheduling results against memory principles:

1. **Row Buffer Hit Harvest Check**:
   * FR-FCFS executed 3 hits on Row 10 back-to-back (Req 2, 4, 6) in Bus Cycles 0, 1, 2.
   * FR-FCFS executed 2 hits on Row 20 back-to-back (Req 1, 5) in Bus Cycles 31, 32.
   * Total `PRECHARGE` commands executed under FR-FCFS = **1 PRECHARGE** (vs. 4 PRECHARGES under FCFS).
2. **Parallel Bank Execution Check**:
   * Req 3 (Bank 1) was activated at Cycle 4 while Bank 0 was opening Row 20.
   * Bank-Level Parallelism hid Bank 1's activation delay completely!
3. **Execution Time Reduction**:
   * Time saved = $107.50\text{ ns} - 29.38\text{ ns} = \mathbf{78.12\text{ nanoseconds saved}}$ ($250\text{ CPU clock cycles}$).

All command queue state evaluations, FR-FCFS priority calculations, row hit harvests, and speedup metrics evaluate with 100% mathematical, physical, and logical precision.
