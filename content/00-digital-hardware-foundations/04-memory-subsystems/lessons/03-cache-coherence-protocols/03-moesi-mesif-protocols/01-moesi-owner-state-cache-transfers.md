content/00-digital-hardware-foundations/04-memory-subsystems/lessons/03-cache-coherence-protocols/03-moesi-mesif-protocols/01-moesi-owner-state-cache-transfers.md
...
## The Modified Library Book and the Designated Custodian: An Everyday Mental Model

To build an intuitive, crystal-clear mental model of the MOESI protocol, the Owner state, and inter-cache line transfers before analyzing state transition matrices and bus transaction waveforms, let us consider an everyday analogy: **The Regional Library System**.

Imagine a regional library system with four branch libraries: Branch 0 (**Core 0**), Branch 1 (**Core 1**), Branch 2 (**Core 2**), and Branch 3 (**Core 3**). Each branch library maintains its own local bookshelf (**Private L1 Cache**).
...
In the basement of the central headquarters sits the **Main Printing Press** (**Main DRAM Memory**).

Suppose an author working at Branch 0 edits Book #42 extensively, adding new research chapters (**Store operations**).
...
### The MOESI Solution: The Designated Custodian (`[O] OWNER` Stamp)

To eliminate these 3-hour trips to the central printing press, the library system adopts the **MOESI Protocol** by introducing a fifth status stamp: **`[O] OWNER` (Designated Custodian)**.

```text
THE FIVE MOESI STATUS STAMPS

 1. [I] INVALID    ──► Book is damaged/empty. Cannot be read.
 2. [S] SHARED     ──► Book is in MULTIPLE branches. Clean! Read-Only!
 3. [E] EXCLUSIVE  ──► Book is in THIS BRANCH ONLY. Clean! (Silent Edit Allowed!)
 4. [M] MODIFIED   ──► Book was EDITED HERE. Sole Owner & Dirty!
 5. [O] OWNER      ──► BOOK WAS EDITED HERE, BUT SHARED WITH OTHERS!
                       Dirty relative to Central Press, but SHARED!
                       This Branch is the DESIGNATED CUSTODIAN!
```

Let us watch how the MOESI protocol handles Branch 1's request for Book #42:

1. Branch 0 holds Book #42 in state **`[M] MODIFIED`**.
2. Branch 1 calls over the intercom asking to read Book #42.
3. Branch 0 takes a photocopy of Book #42 and delivers it directly to Branch 1 across the hallway (**Inter-Cache Line Transfer / Intervention** in 1 minute!).
4. **The Critical Stamping Rule**:
   * Branch 1 stamps its new copy **`[S] SHARED`**.
   * Branch 0 changes its own stamp from **`[M] MODIFIED` to `[O] OWNER`**!
5. **DOES BRANCH 0 DRIVE TO THE CENTRAL PRINTING PRESS? NO!** The central printing press is NOT updated! Zero 3-hour trips are made!

```text
MOESI DESIGNATED CUSTODIAN SEQUENCE (1-MINUTE TRANSFER)

 Branch 1 asks to read Book #42 ──► Branch 0 sends photocopy directly (1 Minute!)
                                    Branch 1 stamps copy [S] SHARED.
                                    Branch 0 stamps copy [O] OWNER.
                                    (Zero trips to Central Press! 3 Hours Saved!)
```

#### What Are the Responsibilities of the `[O] OWNER` Branch?

Branch 0 (holding the **`[O] OWNER`** stamp) becomes the **Designated Custodian** for Book #42 across the entire library system:

1. **Responsibility A (Answering Future Requests)**: If Branch 2 or Branch 3 later asks to read Book #42 over the intercom, **Branch 0 (the Owner) supplies the copy directly**! The central printing press remains completely uninvolved.
2. **Responsibility B (Eventual Writeback)**: Because the central printing press is still outdated, Branch 0 assumes the sole responsibility for eventually driving to the central press and updating the master edition **ONLY when Branch 0 needs to throw Book #42 off its shelf** (**Eviction Writeback**).

This library custodian model is the exact physical analogue of **The MOESI Protocol Owner State**:
* Branch 0 is **CPU Core 0**.
* Branch 1 is **CPU Core 1**.
* The central printing press is **Main System DRAM Memory**.
* Photocopying across the hallway is an **Inter-Cache Line Transfer (Cache-to-Cache Intervention)**.
* The `[O] OWNER` stamp is the **MOESI Owner ($O$) State**.
...
### Formal Definition and Invariants of the Five MOESI States

Let us define the exact properties, permissions, dirtiness status, and sharing invariants for all five MOESI states:

```text
MOESI STATE DEFINITION AND PROPERTY MATRIX

 State Name    │ Encoding [S2,S1,S0] │ Valid? │ Dirty? │ Sharing Status │ Supply Responsibility
───────────────┼─────────────────────┼────────┼────────┼────────────────┼───────────────────────
 [M] Modified  │        3'b111       │ YES    │ YES    │ Private (1)    │ Local Core Only
 [O] Owner     │        3'b110       │ YES    │ YES    │ Shared (>= 2)  │ YES (Supplies Bus!)
 [E] Exclusive │        3'b101       │ YES    │ NO     │ Private (1)    │ Local Core Only
 [S] Shared    │        3'b011       │ YES    │ NO     │ Shared (>= 2)  │ NO (Listens to Bus)
 [I] Invalid   │        3'b000       │ NO     │ NO     │ None (0)       │ None
```

#### 1. Modified State ($M$ — `3'b111`)
* **Properties**: Valid ($V=1$), Dirty ($D=1$), Private (held by $1$ core only).
* **Meaning**: The line was modified by the local core. Main DRAM holds stale data.
* **Permissions**: Local core has full Read and Write permissions in $1\text{ clock cycle}$ with $0\text{ bus traffic}$.

#### 2. Owner State ($O$ — `3'b110`) — THE FIFTH STATE!
* **Properties**: Valid ($V=1$), **Dirty ($D=1$)**, **Shared ($N \ge 2$ cores hold copies)**.
* **Meaning**: The line contains modified data that is **inconsistent with main DRAM memory**, BUT read-only copies of this line are **also held by other cores in the Shared ($S$) state**!
* **Supply Responsibility**: This core is the **Designated Owner**. If any other core requests to read this line over the bus, **this core MUST intercept the request and supply the data directly**!
* **Permissions**: Read-Only locally. If the Owner core wants to write to the line again, it must broadcast a bus invalidation (`BusUpgr`) to invalidate all $S$-copy holders before upgrading $O \to M$.

#### 3. Exclusive State ($E$ — `3'b101`)
* **Properties**: Valid ($V=1$), Clean ($D=0$), Private (held by $1$ core only).
* **Meaning**: The line matches main DRAM, and no other core holds a copy.
* **Permissions**: Read-Only locally, but allows **Silent Local Write Upgrades ($E \to M$)** in $1\text{ clock cycle}$ with $0\text{ bus traffic}$!

#### 4. Shared State ($S$ — `3'b011`)
* **Properties**: Valid ($V=1$), **Clean relative to the Owner or DRAM ($D=0$)**, Shared ($N \ge 2$ cores).
* **Meaning**: The line is a read-only copy. The fresh data is held either by main DRAM or by another core in the Owner ($O$) state.
* **Permissions**: Read-Only locally. Writing requires broadcasting `BusUpgr` to acquire ownership.
...
## Primitive 2: Inter-Cache Line Transfer Mechanics (Intervention)

Now let us examine the physical hardware mechanism that makes the MOESI protocol so extraordinarily fast: **Inter-Cache Line Transfers** (also known as **Cache-to-Cache Interventions**).

### What Is an Inter-Cache Line Transfer?

> An **Inter-Cache Line Transfer (or Cache-to-Cache Intervention)** is a peer-to-peer hardware transaction where a private cache holding a modified or owned memory line ($M$ or $O$ state) intercepts a bus read request from another core and drives the 64-byte data payload directly across the interconnect bus to the requesting core's cache, bypassing main DRAM memory completely.

```text
INTER-CACHE LINE TRANSFER (CACHE-TO-CACHE INTERVENTION)

 Core 0 Private L1 Cache                  Core 1 Private L1 Cache
 Holds Line A in Modified (M) State      Requests Line A (BusRd Miss)
 ┌──────────────────────┐                 ┌──────────────────────┐
 │ L1 SRAM Data Array   │                 │ L1 SRAM Data Array   │
 └──────────┬───────────┘                 └──────────▲───────────┘
            │                                        │
            │  1. Intercepts BusRd!                  │
            │  2. Drives 64B Payload ONTO BUS        │ 3. Captures Payload
            ▼                                        │    in 12 Cycles!
 ═══════════╧════════════════════════════════════════╧════════════════════
                        SHARED MEMORY BUS
                            │
                            ▼ 4. MAIN DRAM READ ABORTED!
                      Main DRAM Memory
                      (Zero DRAM Reads Performed!)
```
...
## Complete MOESI Finite State Machine (FSM) Transitions

To implement MOESI in synthesizable digital hardware, each cache line's 3-bit status register ($S_2, S_1, S_0$) is governed by an expanded 5-state Finite State Machine.

### The Complete MOESI State Transition Matrix

The following comprehensive matrix specifies all state transitions, output actions, and bus transactions for the five MOESI states in response to local CPU requests (`PrRd`, `PrWr`) and snooped bus transactions (`BusRd`, `BusRdX`, `BusUpgr`):

```text
COMPLETE MOESI PROTOCOL TRANSITION MATRIX
...
─────────┼────────────────┼─────────────────┼──────────────────┼────────────────────────────
 ...
 Excl.   │ (0 Bus)        │ -> M (0 Bus!)   │ -> S             │ -> I
─────────┼────────────────┼─────────────────┼──────────────────┼────────────────────────────
 [O]     │ Hit -> O       │ Hit -> BusUpgr  │ INTERVENE!       │ Intervene!
 Owner   │ (0 Bus)        │ -> M            │ Supply Data -> O │ Supply Data -> I
─────────┼────────────────┼─────────────────┼──────────────────┼────────────────────────────
 [M]     │ Hit -> M       │ Hit -> M        │ INTERVENE!       │ Intervene!
 Modified│ (0 Bus)        │ (0 Bus)         │ Supply Data -> O │ Supply Data -> I
```
...
#### 2. Re-Accessing an Owned Line in Local Core ($O \to O$)
...
* **Transition Mechanics**:
  ...
  * State remains **Owner ($O$)**.

$$\text{Local } \mathtt{PrRd} \text{ on } O \text{ State} \implies \text{Local State: } O \longrightarrow \mathbf{O} \quad (\text{1-Cycle L1 Hit})$$
...
#### 4. Upgrading an Owned Line to Modified ($O \to M$)
* **Initial State**: Core 0 holds line $A$ in **Owner ($O$) state**; Cores 1 and 2 hold $A$ in **Shared ($S$) state**.
* **Trigger Event**: Core 0 wants to write to line $A$ again (`PrWr`).
* **Transition Mechanics**:
  * Core 0 holds a dirty copy, but other cores hold read-only copies ($S$). Core 0 must revoke their read permissions!
  ...
  * Core 0 updates line $A$ in SRAM and transitions from **$O \longrightarrow \mathbf{M}$ (Modified)**!

$$\text{Local } \mathtt{PrWr} \text{ on } O \text{ State} \implies \text{Broadcasts } \mathtt{BusUpgr}, \quad \text{Local State: } O \longrightarrow \mathbf{M}$$
...
## Silicon Realities: AMD Opteron, ARM AMBA ACE, and Multi-Socket Servers

In commercial semiconductor engineering, the MOESI protocol is widely deployed in high-performance server processors and multi-core SoC interconnects.

### 1. AMD Opteron and Phenom Direct Connect Architecture

AMD pioneered the commercial deployment of MOESI in x86 server processors starting with the **AMD Opteron** family.

In multi-socket Opteron servers (where 2, 4, or 8 CPU sockets are connected via high-speed point-to-point **HyperTransport** links), main DRAM memory is divided among sockets (Non-Uniform Memory Access / NUMA).

```text
AMD OPTERON MULTI-SOCKET MOESI HYPERTRANSPORT INTERCONNECT

 Socket 0 (Core 0 L1/L2 Cache) ═════ HyperTransport Link ═════► Socket 1 (Core 1 L1/L2 Cache)
 Holds Line A in OWNER (O) State                                 Reads Line A in SHARED (S) State
 (Supplies Line A directly across HyperTransport link without querying off-chip DRAM!)
```

If Socket 0 modified line $A$, and Socket 1 requested to read line $A$:
* Under MESI: Socket 0 would have to write line $A$ back to Socket 0's local DRAM controller over HyperTransport, wait for DRAM to write, and then read DRAM back to Socket 1.
* Under **MOESI**: Socket 0 sets line $A$ to **Owner ($O$)** and streams line $A$ directly across the HyperTransport link to Socket 1! 

MOESI reduced multi-socket memory latency by **over $60\%$**, making multi-socket Opteron servers dominant in data centers throughout the 2000s!
...
## Solved Industrial Engineering Exercise: Quantitative MOESI versus MESI State Machine Execution Trace and DRAM Latency Savings

To consolidate your complete mastery of the MOESI protocol, the Owner ($O$) state, inter-cache line transfers, and DRAM writeback bandwidth savings, we will now walk through a complete, step-by-step industrial hardware engineering problem.
...
##### Op 2 (Core 1 `LOAD A` — MESI FORCED WRITEBACK!):
...
* Write Hit in $S$ state. Core 0 broadcasts `BusUpgr(A)` ($8\text{ cycles}$).
* Cores 1, 2, 3 invalidate ($S \to I$). Core 0 transitions to **Modified ($M$)**.
* **State**: $(S_0=\mathbf{M}, S_1=I, S_2=I, S_3=I)$. Stall = $8\text{ cycles}$.

##### Op 6 (Core 0 Evicts Line $A$):
* Core 0 holds $S_0 = M$ ($D_0 = 1$). Must write back dirty line $A$ to DRAM ($120\text{ cycles}$).
* **State**: $(S_0=I, S_1=I, S_2=I, S_3=I)$. Stall = $120\text{ cycles}$.

```text
MESI PROTOCOL TOTAL STALL CYCLES
= Op1 (120c) + Op2 (120c DRAM Writeback!) + Op3 (12c) + Op4 (12c) + Op5 (8c) + Op6 (120c)
= 392 CLOCK CYCLES TOTAL STALL!
```
...
#### Step 3: Comparative Performance and Bandwidth Analysis

Let us compare MESI vs. MOESI across the 6-operation trace:

```text
MESI VS MOESI PERFORMANCE COMPARISON MATRIX

 Metric                     │ Standard MESI Protocol │ MOESI Protocol         │ MOESI Improvement
────────────────────────────┼────────────────────────┼────────────────────────┼───────────────────────────
 Off-Chip DRAM Writebacks   │ 2 Writebacks (128 B)   │ 1 Writeback (64 B)     │ 50% Less DRAM Write Traffic!
 Op 2 Read Miss Latency     │ 120 Clock Cycles       │ 12 Clock Cycles        │ 90% Faster Inter-Core Read!
 Total Execution Stall Time │ 392 Clock Cycles       │ 284 Clock Cycles       │ 108 Cycles Saved!
 Total Time in Nanoseconds  │ 122.50 ns              │ 88.75 ns               │ 33.75 ns Saved!
 Overall Speedup Factor     │ 1.00x (Base MESI)      │ 1.380x FASTER!         │ 38.0% SPEEDUP!
```

##### Mathematical Speedup Calculation:

$$\text{Speedup}_{\text{MOESI}} = \frac{\text{Stall Cycles}_{\text{MESI}}}{\text{Stall Cycles}_{\text{MOESI}}} = \frac{392\text{ cycles}}{284\text{ cycles}} \approx \mathbf{1.380\times \text{ Performance Advantage!}}$$

```text
MOESI STATE TRANSITION SEQUENCE SUMMARY

 Op 1: Core 0 STORE A ──► Core 0 state = M (Modified)
 Op 2: Core 1 LOAD A  ──► Core 0 Intervenes! Core 0 state = O (Owner), Core 1 = S (Shared)
 Op 3: Core 2 LOAD A  ──► Core 0 Intervenes! Core 2 state = S (Shared)
 Op 4: Core 3 LOAD A  ──► Core 0 Intervenes! Core 3 state = S (Shared)
 Op 5: Core 0 STORE A ──► Core 0 broadcasts BusUpgr! Core 0 state = M, Cores 1,2,3 = I
 Op 6: Core 0 Evicts  ──► Core 0 writes back to DRAM ONCE! Core 0 state = I
```
...
* **MOESI Owner State ($O$)**: The fifth cache line status state in the MOESI protocol ($V=1, D=1, \text{Shared}$) that designates a private cache as the sole custodian of a modified line shared with other cores, allowing the Owner core to supply data directly to peer cores while deferring DRAM writebacks until line eviction.
* **Inter-Cache Line Transfer (Intervention / Cache-to-Cache Transfer)**: The high-speed peer-to-peer forwarding of a 64-byte data line directly from a modified/owned private cache to a requesting core's cache across the interconnect bus ($12\text{ cycles}$), bypassing main DRAM memory and aborting off-chip DRAM read/write operations.
