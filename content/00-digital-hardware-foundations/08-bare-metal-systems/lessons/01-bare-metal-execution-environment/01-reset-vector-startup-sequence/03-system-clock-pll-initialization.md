content/00-digital-hardware-foundations/08-bare-metal-systems/lessons/01-bare-metal-execution-environment/01-reset-vector-startup-sequence/03-system-clock-pll-initialization.md
...
## The Sprinter, the Gearbox, and the Page-Turner: A Mental Model for System Clocking

To build an intuitive, crystal-clear mental model of clock trees, Phase-Locked Loops (PLLs), Flash wait states, and memory remapping before inspecting register bitfields and state machine equations, let us consider an everyday analogy: **A Sprinter Reading a Book**.

Imagine a sprinter (**The CPU Execution Pipeline**) running down a track. Beside the sprinter walks a page-turner carrying a large book (**Non-Volatile Flash ROM**). The sprinter reads one line of text from the book on every stride (**Instruction Fetch Phase**).

```text
THE SPRINTER AND PAGE-TURNER METAPHOR

 Sprinter (CPU Pipeline)                 Page-Turner (Flash ROM Memory)
 ┌───────────────────────────┐           ┌───────────────────────────┐
 │ Takes strides to a rhythm │           │ Turns pages of the book   │
 └─────────────┬─────────────┘           └─────────────▲─────────────┘
               │                                       │
               ▼ Metronome Tempo (System Clock)        │
 ┌─────────────────────────────────────────────────────┴─────┐
 │ Low Speed (RC Oscillator) : Sprinter & Page-Turner match  │
 │ High Speed (PLL Engine)   : Sprinter outruns Page-Turner! │
 └───────────────────────────────────────────────────────────┘
```

Let us observe how the sprinter's performance changes as we adjust their stride speed:
...
### Step 2: The Precision Quartz Metronome and the Mechanical Gearbox (HSE & PLL)

To fix the tempo drift, the sprinter replaces the wooden metronome with an ultra-precise atomic quartz metronome (**External Crystal Oscillator - HSE**) ticking at exactly 8 beats per minute ($8\text{ MHz}$).

To run at top athletic speed, the sprinter attaches a **Mechanical Gearbox (The Phase-Locked Loop - PLL)** to the quartz metronome:
* The gearbox accepts the 8-beat input signal from the quartz metronome.
* Inside the gearbox, an internal multiplier gear multiplies the 8 beats by 21 (**VCO Multiplication Factor $N = 210$, $M = 10$**), generating 168 beats per minute!
* The output gear delivers a lightning-fast, ultra-precise rhythm of **168 beats per minute ($168\text{ MHz}$)** to the sprinter's legs!

```text
THE MECHANICAL GEARBOX (PHASE-LOCKED LOOP)

 Quartz Metronome (8 MHz) ──► [ Divide by 8 (M=8) ] ──► 1 MHz Base
                                                              │
                                                              ▼
 168 MHz Output Rhythm ◄── [ Divide by 2 (P=2) ] ◄── [ Multiply by 336 (N=336) ]
 (Synthesizes 168 MHz from an 8 MHz input crystal with 100% quartz stability!)
```
...
### Step 4: Switching Entrance Signs (Memory Remapping)

Finally, suppose the stadium has three different entrance doors leading to three different books:
1. Door 0 leading to the Main Textbook (**Main Flash ROM**).
2. Door 1 leading to the Factory Repair Manual (**System Bootloader ROM**).
3. Door 2 leading to a Blank Writing Notepad (**Internal SRAM Memory**).

The stadium manager installs an **Electronic Signpost (The Memory Remap Multiplexer)** over the main entrance (`Address 0x0000_0000`):
* By flipping a switch on the signpost (`MEMRMP` register), the manager can make Entrance `0x0000_0000` point directly to the Main Textbook, the Repair Manual, or the Blank Writing Notepad!

This sports car and sprinter system is the exact physical analogue of **System Clock Trees, Phase-Locked Loops, Flash Wait States, and Memory Remapping**:
* The sprinter is the **CPU Execution Pipeline**.
* The page-turner with the book is **Non-Volatile Flash ROM Memory**.
* The wooden metronome is the **Internal RC Oscillator (HSI)**.
* The quartz metronome is the **External Crystal Oscillator (HSE)**.
* The mechanical gearbox is the **Phase-Locked Loop (PLL)**.
* Taking 5 pause-steps per line is inserting **Flash Wait States (`LATENCY = 5`)**.
* The electronic signpost over Entrance `0x0000_0000` is **Hardware Memory Remapping (`SYSCFG_MEMRMP`)**.
...
### 1. The System Clock Tree Architecture

Inside an advanced microcontroller or System-on-Chip (SoC), a single clock frequency does not drive every hardware module. 

Different internal subsystems require different clock frequencies:
* The CPU core and high-speed memory run at the maximum **System Clock ($SYSCLK / HCLK$)** (e.g., $168\text{ MHz}$).
* High-speed peripheral buses (such as PCIe, AHB, and Advanced High-Performance Buses) run at **$HCLK$** ($168\text{ MHz}$).
* Medium-speed peripheral buses (such as APB2 for high-speed UARTs and Timers) run at **$PCLK2$** (e.g., $84\text{ MHz}$).
* Low-speed peripheral buses (such as APB1 for $I^2C$, SPI, and basic Timers) are strictly rate-limited to a lower maximum frequency **$PCLK1$** (e.g., $42\text{ MHz}$).

To distribute clocks across the chip, the hardware incorporates a branching network of frequency dividers called **The Clock Tree**:

```text
BARE-METAL SYSTEM CLOCK TREE DISTRIBUTION MAP

 Oscillator Sources                         Clock Dividers & Buses
 ┌──────────────┐
 │ HSI (16 MHz) ├─┐                         ┌──────────────┐
 └──────────────┘ │  Clock MUX             ┌┤ AHB Prescaler├─► HCLK (168 MHz)
 ┌──────────────┐ ├──►[ SYSCLK MUX ]─► SYSCLK┤ (Divide by 1)│   (CPU Core & RAM)
 │ HSE (8 MHz)  ├─┼───►[ 168 MHz ]   (168 MHz)├──────────────┤
 └──────┬───────┘ │                         │ APB1 Prescaler─► PCLK1 (42 MHz)
        │         │                         │ (Divide by 4)│   (I2C, SPI, UART)
        ▼         │                         ├──────────────┤
 ┌──────────────┐ │                         │ APB2 Prescaler─► PCLK2 (84 MHz)
 │ PLL Engine   ├─┘                         └ (Divide by 2)│   (Fast Timers, ADC)
 └──────────────┘                                          └──────────────┘
```

#### The Four Primary Clock Sources:
1. **HSI (High-Speed Internal)**: Internal $16\text{-MHz}$ RC oscillator. Fast power-on boot source, low precision ($\pm 1\%$).
2. **HSE (High-Speed External)**: External $4\text{ to } 26\text{-MHz}$ quartz crystal oscillator. Ultra-high precision ($\pm 0.001\%$).
3. **LSI (Low-Speed Internal)**: Internal $32\text{-kHz}$ RC oscillator. Low power, used for Watchdog timers and deep sleep.
4. **LSE (Low-Speed External)**: External $32.768\text{-kHz}$ watch crystal. Used for real-time clocks (RTC).
...
### 3. Flash Wait States (Flash Access Latency / `LATENCY[3:0]`)

Because non-volatile Flash ROM memory cells require $20 \text{ to } 30\text{ nanoseconds}$ to complete a read access, the Flash Memory Controller incorporates a **Wait State Pipeline Generator**.

When the CPU system clock ($HCLK$) is accelerated, the software must program the **Flash Access Control Register (`FLASH_ACR`)** to insert hardware wait states (idle clock cycles) into every Flash read access.

```text
FLASH READ TIMING WAVEFORM WITH 0 VS 5 WAIT STATES

 Zero Wait States (HCLK = 16 MHz, T_clk = 62.5 ns)
 Clock (HCLK) : ───┌───┐   ┌───┐   ┌───┐   ┌───┐
                   │ 1 │   │ 2 │   │ 3 │   │ 4 │
 Data Read    : ───[ Valid Data Available ]──── (Data ready in 1 cycle! Passed!)

 Five Wait States (HCLK = 168 MHz, T_clk = 5.95 ns)
 Clock (HCLK) : ───┌───┐   ┌───┐   ┌───┐   ┌───┐   ┌───┐   ┌───┐   ┌───┐
                   │ 1 │   │ 2 │   │ 3 │   │ 4 │   │ 5 │   │ 6 │   │ 7 │
 Wait States  : ───[ WS1 ] [ WS2 ] [ WS3 ] [ WS4 ] [ WS5 ] [ DATA VALID ]
                                                           ▲ Data sampled HERE!
```

#### Mathematical Formula for Required Flash Wait States

Let $T_{\text{flash\_access}}$ be the physical read access time of the Flash memory cells (e.g., $T_{\text{flash\_access}} = 30.0\text{ ns}$).
Let $T_{\text{HCLK}}$ be the clock period of the CPU system clock ($T_{\text{HCLK}} = \frac{1}{f_{\text{HCLK}}}$).

The required number of Flash Wait States ($N_{\text{wait\_states}}$) is:

$$\mathbf{N_{\text{wait\_states}} = \left\lceil \frac{T_{\text{flash\_access}}}{T_{\text{HCLK}}} \right\rceil - 1}$$

Where:
* $\lceil \dots \rceil$ is the ceiling function (rounding up to the nearest integer).
* $T_{\text{flash\_access}}$ is the physical Flash memory access time in nanoseconds.
* $T_{\text{HCLK}}$ is the CPU system clock period in nanoseconds.

```text
FLASH WAIT STATES VS. CPU CLOCK FREQUENCY (3.3V OPERATION)

 System Clock Frequency (f_HCLK) │ Clock Period (T_HCLK) │ Required Flash Wait States
─────────────────────────────────┼───────────────────────┼────────────────────────────
    0 MHz  to  30 MHz            │ >= 33.33 ns           │ 0 Wait States (WS0)
   30 MHz  to  60 MHz            │ 16.67 ns - 33.33 ns   │ 1 Wait State  (WS1)
   60 MHz  to  90 MHz            │ 11.11 ns - 16.67 ns   │ 2 Wait States (WS2)
   90 MHz  to 120 MHz            │  8.33 ns - 11.11 ns   │ 3 Wait States (WS3)
  120 MHz  to 150 MHz            │  6.67 ns -  8.33 ns   │ 4 Wait States (WS4)
  150 MHz  to 168 MHz            │  5.95 ns -  6.67 ns   │ 5 Wait States (WS5)
```

#### Hardware Acceleration: Prefetch Buffer and Instruction Cache
Inserting 5 wait states means every instruction fetch pays a $5\text{-cycle}$ stall penalty! 

To eliminate this stall penalty, high-performance Flash controllers include an **Instruction Cache (`ICEN`)** and a **128-bit Prefetch Buffer (`PRFTEN`)**. 

The prefetch buffer fetches 4 consecutive 32-bit instructions ahead of the CPU pipeline in a single wide read, hiding Flash latency and restoring near $1\text{-cycle}$ execution speed!
...
### 5. Memory Remapping Hardware Mechanics (`SYSCFG_MEMRMP`)

When a CPU boots, it fetches its initial Stack Pointer ($SP$) and Program Counter ($PC$) from physical address `0x0000_0000`.

In modern SoCs, physical memory banks are located at separate high-order addresses:
* Main Flash Memory (ROM) is located at `0x0800_0000`.
* Factory System Bootloader ROM is located at `0x1FFF_0000`.
* Internal SRAM (RAM) is located at `0x2000_0000`.

How does the CPU read the vector table at physical address `0x0000_0000` if Flash memory is located at `0x0800_0000`?

The hardware incorporates a **Memory Remapping Multiplexer (`SYSCFG_MEMRMP`)** positioned at the front of the internal bus crossbar:

```text
MEMORY REMAPPING HARDWARE MULTIPLEXER (MEMRMP)

 Physical Memory Banks                                 Memory Map Address 0x0000_0000
 ┌───────────────────────────┐                         ┌───────────────────────────┐
 │ Main Flash (0x0800_0000)  ├─[00]─┐                  │ Physical Base Address     │
 ├───────────────────────────┤      │                  │ 0x0000_0000               │
 │ System ROM (0x1FFF_0000)  ├─[01]─┼─►[ MEMRMP MUX ]─►│ Aliases Selected Bank!    │
 ├───────────────────────────┤      │   (MEM_MODE)     │                           │
 │ SRAM RAM   (0x2000_0000)  ├─[10]─┘                  └───────────────────────────┘
 └───────────────────────────┘
```

By writing to the **System Configuration Memory Remap Register (`SYSCFG_MEMRMP`)**, software can dynamically change which physical memory bank is aliased to address `0x0000_0000`:

* **`MEM_MODE = 2'b00` (Main Flash Memory Remap — Default)**:
  Address `0x0000_0000` aliases physical Flash memory (`0x0800_0000`). Reads from `0x0000_0000` read Flash ROM.
* **`MEM_MODE = 2'b01` (System Flash Memory Remap)**:
  Address `0x0000_0000` aliases factory System ROM (`0x1FFF_0000`), executing the vendor's built-in UART/USB bootloader.
* **`MEM_MODE = 2'b10` (Embedded SRAM Remap)**:
  Address `0x0000_0000` aliases internal SRAM (`0x2000_0000`). 

#### Why Remap SRAM to Address `0x0000_0000`?
Aliasing SRAM to `0x0000_0000` allows advanced bare-metal systems to **modify the interrupt vector table dynamically in RAM at runtime**! 

The application copies a new vector table into SRAM, updates `MEM_MODE = 2'b10`, and the CPU fetches new interrupt vector addresses from high-speed SRAM in $1\text{ single clock cycle}$, completely bypassing Flash ROM delays!
...
### 1. The Over-Clocked Peripheral Bus Crash (APB1 Frequency Violation)

Consider an engineer configuring the clock tree for a $168\text{-MHz}$ processor system clock ($SYSCLK = 168\text{ MHz}$).

The peripheral bus **APB1** (which powers $I^2C$, SPI, and UART communication controllers) has a **maximum physical frequency limit of $42\text{ MHz}$**.

The engineer forgets to set the APB1 Clock Prescaler (`PPRE1`) and leaves it at its default "Divide-by-1" setting:
```text
SYSCLK (168 MHz) ──► APB1 Prescaler (/1) ──► APB1 Clock = 168 MHz! (MAX LIMIT IS 42 MHZ!)
```

Trace the physical hardware failure:
1. APB1 bus clock operates at $168\text{ MHz}$, exceeding its $42\text{-MHz}$ physical silicon rating.
2. The internal flip-flops and bus drivers inside $I^2C$ and UART peripherals suffer timing violations.
3. When the CPU attempts to read a UART status register over APB1, the peripheral returns corrupted bits.
4. The CPU hangs waiting for a UART ready flag that never sets, locking up the communication pipeline!

#### The Hardware Fix:
Software **MUST** configure `PPRE1 = 3'b101` (**Divide-by-4**) before boosting SYSCLK to $168\text{ MHz}$:

$$f_{\text{PCLK1}} = \frac{168\text{ MHz}}{4} = \mathbf{42 \text{ MHz}} \quad (\mathbf{\text{WITHIN 42-MHZ SPECIFICATION!}})$$
...
## Solved Industrial Engineering Exercise: Quantitative Clock Tree Synthesis, PLL Multiplier Calculation, and Assembly Initialization

To consolidate your complete mastery of clock tree architecture, Phase-Locked Loop frequency synthesis math, Flash wait state calculations, and safe clock transition state machines, we will now walk through a complete, step-by-step industrial hardware engineering problem.
...
#### Step 1: Calculate PLL Dividers ($M, N, P$)

##### 1. Calculate Input Divider $M$:
We require $f_{\text{ref}} = 1.000\text{ MHz}$:

$$f_{\text{ref}} = \frac{f_{\text{input}}}{M} = \frac{8.000\text{ MHz}}{M} = 1.000\text{ MHz} \implies \mathbf{M = 8}$$

##### 2. Determine Main Divider $P$ and VCO Frequency $f_{\text{VCO}}$:
To minimize jitter, we choose the smallest main divider $P = 2$:

$$f_{\text{VCO}} = f_{\text{SYSCLK}} \times P = 168.000\text{ MHz} \times 2 = \mathbf{336.000 \text{ MHz}}$$

Verify $f_{\text{VCO}}$ silicon bounds ($100\text{ MHz} \le f_{\text{VCO}} \le 432\text{ MHz}$):

$$100.0\text{ MHz} \le 336.0\text{ MHz} \le 432.0\text{ MHz} \quad (\mathbf{\text{VCO BOUNDS VERIFIED!}})$$

##### 3. Calculate VCO Multiplication Factor $N$:

$$f_{\text{VCO}} = f_{\text{ref}} \times N \implies 336.000\text{ MHz} = 1.000\text{ MHz} \times N \implies \mathbf{N = 336}$$

Verify $N$ silicon bounds ($50 \le N \le 432$): $50 \le 336 \le 432$ ($\mathbf{\text{VERIFIED}}$).

```text
PLL DIVIDER SYNTHESIS RESULTS

 Input Crystal Frequency f_input : 8.000 MHz
 Division Factor M               : 8   (f_ref = 8 / 8 = 1.000 MHz)
 Multiplication Factor N          : 336 (f_vco = 1.000 x 336 = 336.000 MHz)
 Output Division Factor P        : 2   (f_SYSCLK = 336 / 2 = 168.000 MHz)
```
...
#### Step 3: Calculate Required Flash Wait States (`FLASH_ACR.LATENCY`)

Given:
* System Clock Frequency $f_{\text{HCLK}} = 168.000\text{ MHz}$.
* $T_{\text{HCLK}} = \frac{1}{168 \times 10^6\text{ Hz}} \approx 5.95238 \times 10^{-9}\text{ s} = \mathbf{5.95238 \text{ nanoseconds}}$.
* Physical Flash Access Time $T_{\text{flash\_access}} = 30.0\text{ nanoseconds}$.

We apply the Flash Wait State Formula:

$$N_{\text{wait\_states}} = \left\lceil \frac{T_{\text{flash\_access}}}{T_{\text{HCLK}}} \right\rceil - 1$$

$$N_{\text{wait\_states}} = \left\lceil \frac{30.0\text{ ns}}{5.95238\text{ ns}} \right\rceil - 1 = \lceil 5.0399 \rceil - 1 = 6 - 1 = \mathbf{5 \text{ Wait States}}$$

##### Flash Control Register Setting:
Software MUST set `FLASH_ACR.LATENCY = 5` (`3'b101` $= 5\text{ Wait States}$ / 6 CPU cycles per Flash access) before switching the clock MUX to $168\text{ MHz}$!
...
Let us verify our mathematical, physical, and state machine results against silicon specifications:

1. **PLL Frequency Synthesis Verification**:
   * $f_{\text{ref}} = 8.0\text{ MHz} / 8 = 1.000\text{ MHz}$ (Valid reference frequency!).
   * $f_{\text{VCO}} = 1.0\text{ MHz} \times 336 = 336.000\text{ MHz}$ ($100 \le f_{\text{VCO}} \le 432\text{ MHz}$ verified!).
   * $f_{\text{SYSCLK}} = 336.0\text{ MHz} / 2 = \mathbf{168.000 \text{ MHz}}$ (Exact match!).

2. **Peripheral Bus Frequency Compliance**:
   * $f_{\text{HCLK}} = 168.000\text{ MHz} / 1 = 168.000\text{ MHz}$.
   * $f_{\text{PCLK1}} = 168.000\text{ MHz} / 4 = 42.000\text{ MHz} \le 42.000\text{ MHz}$ max ($\mathbf{\text{PASSED!}}$).
   * $f_{\text{PCLK2}} = 168.000\text{ MHz} / 2 = 84.000\text{ MHz} \le 84.000\text{ MHz}$ max ($\mathbf{\text{PASSED!}}$).

3. **Flash Access Timing Margin Verification**:
   * At $168\text{ MHz}$, $T_{\text{HCLK}} = 5.952\text{ ns}$.
   * With 5 wait states, total Flash access window $= (1 + 5) \times 5.952\text{ ns} = \mathbf{35.712 \text{ nanoseconds}}$.
   * Physical Flash requirement $= 30.0\text{ ns}$.
   * Safety Margin $= 35.712\text{ ns} - 30.0\text{ ns} = \mathbf{+5.712 \text{ nanoseconds}}$ positive margin!
   * The Flash controller settles completely before data is sampled, guaranteeing zero `HardFault` crashes!

All PLL divider calculations, bus prescaler limits, Flash wait state timing margins, and assembly clock transition state steps evaluate with 100% mathematical, physical, and logical precision.
