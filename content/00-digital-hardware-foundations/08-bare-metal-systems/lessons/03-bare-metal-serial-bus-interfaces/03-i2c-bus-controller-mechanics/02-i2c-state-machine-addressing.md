content/00-digital-hardware-foundations/08-bare-metal-systems/lessons/03-bare-metal-serial-bus-interfaces/03-i2c-bus-controller-mechanics/02-i2c-state-machine-addressing.md
...
## The Hotel File Cabinet and the Hand on the Metronome: A Mental Model for I2C

To build an intuitive, crystal-clear mental model of stateful register reads, Repeated START conditions, clock stretching, and manual 9-clock bus recovery before inspecting memory-mapped status registers, hardware state diagrams, and assembly routines, let us consider an everyday analogy: **The Hotel Manager and the Guest Room File Cabinet**.

Imagine a hotel manager (**The $I^2C$ Master**) who needs to read a document stored inside File Cabinet #28 (**Register Address `0x28`**) located inside Guest Room #68 (**Slave Address `0x68`**).

```text
THE HOTEL FILE CABINET METAPHOR

 Hotel Manager (I2C Master)                   Guest Room #68 (I2C Slave)
 ┌───────────────────────────┐                ┌───────────────────────────┐
 │ Wants to read Document #28│                │ Contains File Cabinet #28 │
 └─────────────┬─────────────┘                └─────────────▲─────────────┘
               │                                            │
               ▼ Single Hallway Phone Line (SDA / SCL Bus)  │
 ┌──────────────────────────────────────────────────────────┴─────────────┐
 │ PHONE LINE RULES                                                       │
 │  1. Ring Doorbell (START) -> Specify Room #68 + Write Mode             │
 │  2. State Document Number #28                                          │
 │  3. Re-Ring Doorbell (Repeated START) -> Switch to Read Mode           │
 └────────────────────────────────────────────────────────────────────────┘
```

Let us observe three operational scenarios encountered by the hotel manager:
...
### Scenario 2: The Hand on the Metronome (Clock Stretching)

Now, suppose the guest in Room #68 needs 10 seconds to search through File Cabinet #28 to find the document (**Slow Slave Processing / Internal Conversion Delay**).

The manager is playing a rhythmic metronome (**The Master Serial Clock `SCL`**).

How does the slow guest pause the manager so the manager doesn't demand the document before it is found?

* The guest reaches out, grabs the metronome pendulum, and **holds it still at the bottom of its swing (Pulls `SCL` Line Low to $0.0\text{V}$)**!
* The metronome stops ticking.
* The manager does not get confused or crash. The manager sees the metronome is held still and **waits patiently**!
* When the guest finds Document #28, they release the pendulum (**`SCL` floats High to $3.3\text{V}$**). The metronome resumes ticking, and the manager receives the document!

```text
CLOCK STRETCHING (GUEST HOLDS METRONOME STILL)

 Metronome Ticking (SCL Clock) ──► Guest holds pendulum (Pulls SCL Low = 0V)
                                   │
                                   ▼ Metronome FROZEN! Manager WAITS!
 Guest finds Document #28       ──► Guest releases pendulum (SCL floats High = 3.3V)
                                   Metronome resumes ticking! Document transferred!
```
...
## Deep Mechanics of Master Hardware States, Clock Stretching, and GPIO Recovery

Now that we possess an intuitive mental model of hotel file cabinets, metronome pausing, and 9-push un-jamming protocols, let us examine the formal, rigorous engineering mechanics of **$I^2C$ Master Hardware States**, **Clock Stretching**, and **GPIO Bus Recovery**.
...
### 2. Repeated START ($Sr$) Condition Mechanics

When reading a specific register from an $I^2C$ sensor, why must software issue a **Repeated START ($Sr$)** instead of a STOP ($P$) followed by a START ($S$)?

```text
REPEATED START (Sr) VS. STOP-START BUS CONTROL

 Option A: STOP followed by START (UNSAFE MULTI-MASTER HAZARD!)
 Master writes Reg Addr ──► Issue STOP (P) ──► Issue START (S) ──► Read Data
                            ▲
                            └── BUS IS RELEASED! Another Master steals bus HERE!
                                Target register pointer corrupted by other Master!

 Option B: Repeated START (Sr) (SAFE CONTINUOUS BUS CONTROL!)
 Master writes Reg Addr ──► Issue Repeated START (Sr) ──► Read Data
                            ▲
                            └── BUS IS NEVER RELEASED! Continuous Ownership!
```

* **Option A (STOP then START)**:
  When the Master issues a STOP condition ($P$), it releases `SDA` and `SCL` to High-Z ($3.3\text{V}$). The bus returns to Idle state.
  If another Master device is waiting on the bus, it will instantly grab the bus and issue its own START condition, **overwriting the internal register pointer inside the sensor**!
* **Option B (Repeated START $Sr$)**:
  The Master sets `START = 1` in `I2C1_CR1` **while the bus is actively owned**. 
  
  The hardware generates a new START condition on `SDA`/`SCL` **without ever generating a STOP condition or releasing the bus to Idle state**!
  
  Bus ownership remains $100\%$ locked by the Master, guaranteeing thread-safe, atomic multi-register reads.
...
### 4. The 9-Clock Manual GPIO Bus Recovery Sequence

When a host CPU reboots mid-read and finds `SDA` stuck Low ($0.0\text{V}$) because an un-reset slave is waiting for clock pulses:

The $I^2C$ hardware peripheral cannot generate a START condition because it detects `SDA = 0` (Bus Busy).

To un-jam the bus, the bare-metal assembly startup routine executes **The 9-Clock GPIO Bus Recovery Sequence**:

```text
9-CLOCK GPIO BUS RECOVERY ALGORITHM

 1. Disable I2C Peripheral (I2C1_CR1.PE = 0)
 2. Re-configure SCL and SDA pins as General Purpose GPIO Outputs / Inputs
 3. Read SDA pin state -> Is SDA Low (0.0V)?
    ┌────────────────────────┴────────────────────────┐
    ▼ YES (Bus Stuck!)                                ▼ NO (Bus OK!)
 4. Toggle SCL High -> Low -> High 9 Times!          Proceed to I2C Init!
    (Forces stuck slave to shift out remaining bits)
 5. Generate Manual START & STOP via GPIO BSRR!
 6. Re-configure SCL and SDA as Alternate Function (AF4)
 7. Enable I2C Peripheral (I2C1_CR1.PE = 1) -> BUS RECOVERED!
```

```text
9-CLOCK BUS RECOVERY WAVEFORM SEQUENCE

 SCL GPIO Output: ─┌─┐─┌─┐─┌─┐─┌─┐─┌─┐─┌─┐─┌─┐─┌─┐─┌─┐────────
                  │1│ │2│ │3│ │4│ │5│ │6│ │7│ │8│ │9│
 SDA Line       : ──┴───┴───┴───┴───┴───┴───┴───┴───┼──────── (SDA floats High!)
                                                     ▲
                                                     └── Slave releases SDA on 9th clock!
```

#### Why 9 Clock Pulses?
An $I^2C$ byte frame consists of 8 Data Bits + 1 ACK/NACK Bit ($9\text{ total clock pulses}$). 

No matter which bit position ($0 \dots 8$) the slave was stuck on when the CPU rebooted, toggling `SCL` 9 times guarantees that the slave will reach the 9th bit, release `SDA` High to $3.3\text{V}$, and return to its Idle state!
...
## Solved Industrial Engineering Exercise: Quantitative I2C Master State Machine Trace, Clock Stretching Timing Analysis, and Assembly Driver Synthesis

To consolidate your complete mastery of $I^2C$ master hardware state machines, Repeated START ($Sr$) generation, clock stretching timing bounds, 9-clock bus recovery sequences, and assembly driver synthesis, we will now walk through a complete, step-by-step industrial hardware engineering problem.
...
#### Step 1: Calculate 8-Bit Write and Read Address Bytes

7-bit Slave Address $= \text{0x68} = 1101\_000_2$.

##### 1. Write Address Byte ($\text{Addr}_{\text{write}}$ — Bit $0 = 0$):

$$\text{Addr}_{\text{write}} = (\text{0x68} \ll 1) \mid 0 = (1101\_0000_2) \mid 0 = \mathbf{1101\_0000_2} = \mathbf{\text{0xD0}}$$

##### 2. Read Address Byte ($\text{Addr}_{\text{read}}$ — Bit $0 = 1$):

$$\text{Addr}_{\text{read}} = (\text{0x68} \ll 1) \mid 1 = (1101\_0000_2) \mid 1 = \mathbf{1101\_0001_2} = \mathbf{\text{0xD1}}$$
...
#### Step 3: Calculate Clock Stretching Hardware Wait Duration

Given:
* Sensor Clock Stretching Duration $T_{\text{stretch}} = 15.0\text{ microseconds} = 15,000\text{ ns}$.
* Host CPU Clock Frequency $f_{\text{clk}} = 3.2\text{ GHz} \implies T_{\text{clk}} = 0.3125\text{ ns}$.

Calculate CPU clock cycles elapsed during the $15.0\text{-}\mu\text{s}$ hardware clock stretch:

$$\text{Cycles}_{\text{stretch}} = \frac{T_{\text{stretch}}}{T_{\text{clk}}} = \frac{15,000\text{ ns}}{0.3125\text{ ns/cycle}} = \mathbf{48,000 \text{ CPU Clock Cycles}}$$

During the $15.0\ \mu\text{s}$ clock stretch, the $I^2C$ hardware freezes `SCL` automatically. If software is polling `BTF` or `RxNE`, the polling loop waits safely for **$48,000\text{ CPU clock cycles}$** until the sensor releases `SCL`, with zero data corruption!
...
#### Step 5: Write Production Assembly 9-Clock Bus Recovery Routine (`I2C1_BusRecovery`)

Here is the complete assembly routine that manually clocks `SCL` 9 times via GPIO bit-banding to clear a stuck `SDA` line:

```assembly
/* PRODUCTION BARE-METAL 9-CLOCK I2C BUS RECOVERY IN ASSEMBLY */
.syntax unified
.cpu cortex-m4
.thumb

.equ GPIOB_BASE,      0x40020400
.equ GPIOB_MODER,     0x40020400        /* Mode Register */
.equ GPIOB_IDR,       0x40020410        /* Input Data Register */
.equ GPIOB_BSRR,      0x40020418        /* Bit Set/Reset Register */
.equ GPIOB_AFRL,      0x40020420        /* AF Low Register */

.equ I2C1_CR1_ADDR,   0x40005400        /* I2C1 Control Register 1 */

.global I2C1_BusRecovery
.type I2C1_BusRecovery, %function
...
I2C1_BusRecovery:
    ...
    /* Step 1: Disable I2C1 Peripheral (PE = 0) */
    ldr     r0, =I2C1_CR1_ADDR
    ldr     r1, [r0]
    bic     r1, r1, #(1 << 0)           /* Clear PE bit (Bit 0) */
    str     r1, [r0]

    /* Step 2: Check if SDA (PB7) is stuck Low (PB7 == 0) */
    ldr     r0, =GPIOB_IDR
    ldr     r1, [r0]
    tst     r1, #(1 << 7)               /* Test PB7 (SDA line) */
    bne     recovery_done               /* If SDA is High (1), bus is OK! Exit! */

    /* Step 3: Re-configure SCL (PB6) as GPIO Output (MODER = 2'b01) */
    ldr     r0, =GPIOB_MODER
    ldr     r1, [r0]
    bic     r1, r1, #(0x3 << 12)        /* Clear PB6 MODER bits [13:12] */
    orr     r1, r1, #(0x1 << 12)        /* Set PB6 MODER = 2'b01 (Output) */
    str     r1, [r0]

    /* Step 4: Toggle SCL (PB6) Low -> High 9 Times */
    movs    r4, #9                      /* Counter = 9 pulses */
    ldr     r5, =GPIOB_BSRR

clock_recovery_loop:
    /* SCL Low (PB6 = 0) */
    ldr     r6, =(1 << (6 + 16))        /* Reset PB6 */
    str     r6, [r5]
    bl      delay_5us                   /* Wait 5 microseconds */
    ...
    /* Check if SDA (PB7) has floated back High */
    ldr     r0, =GPIOB_IDR
    ldr     r1, [r0]
    tst     r1, #(1 << 7)               /* Test PB7 (SDA) */
    bne     bus_cleared                 /* SDA released! Exit loop! */

    subs    r4, r4, #1
    bne     clock_recovery_loop

bus_cleared:
    /* Step 5: Restore SCL (PB6) to Alternate Function AF4 (MODER = 2'b10) */
    ldr     r0, =GPIOB_MODER
    ldr     r1, [r0]
    bic     r1, r1, #(0x3 << 12)
    orr     r1, r1, #(0x2 << 12)        /* Set PB6 MODER = 2'b10 (AF4) */
    str     r1, [r0]

recovery_done:
    /* Step 6: Re-enable I2C1 Peripheral (PE = 1) */
    ldr     r0, =I2C1_CR1_ADDR
    ldr     r1, [r0]
    orr     r1, r1, #(1 << 0)           /* Set PE = 1 */
    str     r1, [r0]

    pop     {r4, r5, r6, pc}

delay_5us:
    movs    r0, #160                    /* 160 loops * 2c = 320c @ 3.2 GHz = 100 ns? */
    /* Adjust for 5 us = 16,000 CPU cycles at 3.2 GHz */
    ldr     r0, =16000
delay_loop:
    subs    r0, r0, #1
    bne     delay_loop
    bx      lr
.size I2C1_BusRecovery, .-I2C1_BusRecovery
```
...
* **$I^2C$ Master State Machine**: The hardware and assembly state controller that steps through `SB` (Start Bit Generated), `ADDR` (Address Acknowledged), `TxE`/`RxNE`, and `BTF` (Byte Transfer Finished) status flags to orchestrate complex multi-byte write/read transactions over $I^2C$.
* **Clock Stretching Detection**: The open-drain bus mechanism where a slow slave device holds the `SCL` clock line Low ($0.0\text{V}$) to force the Master's clock generator into a wait state until the slave is ready to transmit or receive the next byte.
* **Bus Recovery Sequence**: The bare-metal recovery algorithm where the CPU re-configures `SCL` as a GPIO output and manually toggles `SCL` 9 times to force a stuck slave device to release the `SDA` line back to High-Z state (`1`), recovering from stuck-bus deadlocks.