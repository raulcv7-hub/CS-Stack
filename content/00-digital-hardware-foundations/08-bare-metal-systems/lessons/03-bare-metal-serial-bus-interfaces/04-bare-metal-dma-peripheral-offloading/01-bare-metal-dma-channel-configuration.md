...
content/00-digital-hardware-foundations/08-bare-metal-systems/lessons/03-bare-metal-serial-bus-interfaces/04-bare-metal-dma-peripheral-offloading/01-bare-metal-dma-channel-configuration.md
...
## The Conveyor Belt and the Two-Bucket Relay: A Mental Model for Bare-Metal DMA

To build an intuitive, crystal-clear mental model of bare-metal DMA channels, circular memory pointers, ping-pong double buffering, and half-transfer versus transfer-complete interrupts before inspecting memory-mapped registers and assembly equations, let us consider an everyday analogy: **The Automated Bottling Plant**.

Imagine a factory manager (**The CPU Execution Core**) working in a high-speed bottling facility. An automated beverage machine (**A Peripheral Sensor / ADC / SPI**) fills glass bottles with soda at a rate of 1 bottle per second (**High-Bandwidth Sensor Streaming**).

```text
THE AUTOMATED BOTTLING PLANT METAPHOR

 Beverage Machine (Peripheral ADC / SPI)       Factory Manager (CPU Core)
 ┌───────────────────────────┐                 ┌───────────────────────────┐
 │ Fills 1 Bottle / Second   │                 │ Executes Quality Control  │
 │ (High-Speed Sensor Data)  │                 │ & Business Management     │
 └─────────────┬─────────────┘                 └─────────────▲─────────────┘
               │                                             │
               ▼ (Fills Bottles Continuously)                │
 ┌───────────────────────────────────────────────────────────┴─────────────┐
 │ AUTOMATED MOTOR-DRIVEN CONVEYOR BELT (Bare-Metal DMA Channel)           │
 │ Moves bottles directly from Machine to Storage Crate without Manager!   │
 └─────────────────────────────────────────────────────────────────────────┘
```

The filled bottles must be transported from the machine nozzle to a storage crate (**Main System RAM**) so the manager can inspect them.

Let us compare two operational strategies for moving these bottles:
...
### Strategy 2: The Automated Conveyor Belt (Bare-Metal DMA Channel)

To liberate the manager, the factory installs an **Automated Motor-Driven Conveyor Belt (A Bare-Metal DMA Channel)** running directly from the machine nozzle to the storage crate.

The manager programs the conveyor belt's mechanical control box once on Monday morning (**DMA MMIO Configuration**):
* **Source Address (`PAR`)**: Beverage Machine Nozzle #1.
* **Destination Address (`MAR`)**: Storage Crate Slot #0 in the warehouse.
* **Transfer Count (`NDTR`)**: 1,000 Bottles.

```text
AUTOMATED CONVEYOR BELT (BARE-METAL DMA CHANNEL)

 Manager programs Conveyor Box ONCE (Takes 2 Seconds) ──► Returns to Office Desk!
                                                          │
                                                          ▼ (Conveyor moves 1,000 bottles)
 Conveyor Belt moves bottles in background ───────────────► Zero Manager Walking!
```

1. The manager presses the "START" button and **immediately returns to their office desk to run the business**!
2. In the background, the motorized conveyor belt carries bottles from the machine nozzle directly into the storage crate at full physical speed.
3. **ZERO MANAGER EFFORT!** The manager executes zero walking steps while 1,000 bottles move into the warehouse!
...
## Deep Mechanics of DMA Channels, Circular Mode, and HT/TC Interrupts

Now that we possess an intuitive mental model of conveyor belts and two-bucket relays, let us examine the formal, rigorous engineering mechanics of **Bare-Metal DMA Channels**, **Circular Mode (`CIRC`)**, and **Half-Transfer (`HT`) / Transfer-Complete (`TC`) Interrupts**.
...
### 2. Dissecting the Channel Configuration Register (`DMA_CCRx` / `DMA_SxCR`)

The **Channel Configuration Register (`DMA_CCRx`)** controls transfer direction, data sizes, pointer increment rules, circular mode, and interrupt triggers:

```text
DMA_CCRx CONFIGURATION REGISTER BITFIELD MAP

 Bit 31               Bit 14 Bit 13 Bit 11 Bit 9 Bit 7  Bit 6  Bit 5  Bit 4 Bit 3 Bit 2 Bit 1 Bit 0
 ┌───────────────────┬──────┬──────┬──────┬──────┬──────┬──────┬──────┬─────┬─────┬─────┬─────┬─────┐
 │ Reserved          │ MEM2 │ PL   │ MSIZE│ PSIZE│ MINC │ PINC │ CIRC │ DIR │ TEIE│ HTIE│ TCIE│ EN  │
 │ (Read as 0)       │ MEM  │ (2b) │ (2b) │ (2b) │ (1b) │ (1b) │ (1b) │ (1b)│(1b) │(1b) │(1b) │(1b) │
 └───────────────────┴──────┴──────┴──────┴──────┴──────┴──────┴──────┴─────┴─────┴─────┴─────┴─────┘
```

* **`EN` (Channel Enable — Bit 0)**:
  * $0 =$ DMA Channel disabled (paused).
  * $1 =$ DMA Channel enabled (actively listening for peripheral DMA requests).
* **`TCIE` (Transfer Complete Interrupt Enable — Bit 1)**:
  * $1 =$ Asserts a hardware interrupt (`TCIF = 1`) when the down-counter `CNDTR` reaches zero ($0$).
* **`HTIE` (Half Transfer Interrupt Enable — Bit 2)**:
  * $1 =$ Asserts a hardware interrupt (`HTIF = 1`) when `CNDTR` reaches **exact half-capacity ($\frac{N_{\text{total}}}{2}$)**.
* **`DIR` (Data Transfer Direction — Bit 4)**:
  * $0 =$ **Read from Peripheral, Write to Memory** (Peripheral-to-RAM, e.g., ADC or SPI Read).
  * $1 =$ **Read from Memory, Write to Peripheral** (RAM-to-Peripheral, e.g., DAC or SPI Write).
* **`CIRC` (Circular Mode Enable — Bit 5)**:
  * $0 =$ Normal linear mode. The DMA channel stops when `CNDTR` reaches 0.
  * $1 =$ **Circular Mode Active!** When `CNDTR` reaches 0, the hardware automatically reloads `CNDTR` with its initial total value and resets `CMAR` back to the starting RAM buffer address in $0\text{ clock cycles}$!
* **`PINC` (Peripheral Increment Mode — Bit 6)**:
  * $0 =$ Fixed Peripheral Address (Target address `CPAR` stays constant on every transfer, e.g., always reading `ADC1_DR`).
* **`MINC` (Memory Increment Mode — Bit 7)**:
  * $1 =$ **Increment Memory Address** (`CMAR` automatically increments by 1, 2, or 4 bytes after every transfer so subsequent samples write to consecutive RAM array slots).
* **`PSIZE[1:0]` & `MSIZE[1:0]` (Data Size — Bits $[9:8]$ and $[11:10]$)**:
  * `2'b00` $= 8\text{-bit}$ Byte transfer.
  * `2'b01` $= 16\text{-bit}$ Half-Word transfer (standard for 12-bit ADCs).
  * `2'b10` $= 32\text{-bit}$ Word transfer.
...
## Ping-Pong Double Buffering and HT/TC Interrupt Mechanics

Now let us examine the core microarchitectural primitive that enables $100\%$ continuous, zero-copy data streaming: **Ping-Pong Double Buffering** driven by **Half-Transfer (`HT`)** and **Transfer-Complete (`TC`)** interrupts.

### Allocating the Circular Double Buffer in Memory

To implement Ping-Pong double buffering, software allocates a single contiguous array in RAM containing $N_{\text{total}}$ element slots (where $N_{\text{total}}$ is an even integer, e.g., $N_{\text{total}} = 200$ samples):

```c
/* PING-PONG DOUBLE BUFFER ARRAY IN SYSTEM RAM */
uint16_t adc_buffer[200];  /* 200 Half-Words = 400 Bytes Total */
```

The hardware and software treat this single 200-element array as **Two Equal Half-Buffers**:

$$\text{Half-Buffer 0 (Ping Buffer)} = \text{\&adc\_buffer}[0] \dots \text{\&adc\_buffer}[99] \quad (\text{Slots } 0 \dots 99)$$

$$\text{Half-Buffer 1 (Pong Buffer)} = \text{\&adc\_buffer}[100] \dots \text{\&adc\_buffer}[199] \quad (\text{Slots } 100 \dots 199)$$

$$\text{Half-Buffer Size } N_{\text{half}} = \frac{N_{\text{total}}}{2} = \frac{200}{2} = \mathbf{100 \text{ Samples}}$$

```text
PING-PONG DOUBLE BUFFER MEMORY LAYOUT

 RAM Array: uint16_t adc_buffer[200]
 ┌──────────────────────────────────────┬──────────────────────────────────────┐
 │ HALF-BUFFER 0 (Ping Buffer)          │ HALF-BUFFER 1 (Pong Buffer)          │
 │ Slots 0 .. 99 (100 Samples / 200B)   │ Slots 100 .. 199 (100 Samples / 200B)│
 └──────────────────────────────────────┴──────────────────────────────────────┘
  ◄── CNDTR counts 200 down to 100 ────► ◄── CNDTR counts 100 down to 0 ──────►
  ▲                                      ▲
  │                                      │
  HTIF Interrupt Fires at CNDTR = 100!   TCIF Interrupt Fires at CNDTR = 0!
  (DMA moves to Buffer 1; CPU reads Buf 0)(DMA wraps to Buffer 0; CPU reads Buf 1)
```
...
### Hardware Interrupt Status and Clear Registers (`DMA_ISR` / `DMA_IFCR`)

To manage DMA interrupts, the hardware provides two 32-bit registers:
1. **Interrupt Status Register (`DMA_ISR`)**: Holds read-only status flags (`TCIFx`, `HTIFx`, `TEIFx` for Transfer Error, `GIFx` for Global Interrupt).
2. **Interrupt Flag Clear Register (`DMA_IFCR`)**: Write-only register used to clear status flags.

```text
DMA_ISR INTERRUPT STATUS REGISTER BITMAP (CHANNEL 1)

 Bit 31                     Bit 4  Bit 3  Bit 2  Bit 1  Bit 0
 ┌─────────────────────────┬──────┬──────┬──────┬──────┬──────┐
 │ Reserved / Channel 2..7 │ Reserved    │ TEIF1│ HTIF1│ TCIF1│ GIF1 │
 └─────────────────────────┴─────────────┴──────┴──────┴──────┴──────┘
  (Bit 2 = HTIF1 Half-Transfer Flag | Bit 1 = TCIF1 Transfer-Complete Flag)
```

#### The Write-1-to-Clear (`W1C`) Rule in `DMA_IFCR`:
To clear a DMA interrupt flag in hardware, software **MUST write a $1$ to the corresponding bit in `DMA_IFCR`**:

```assembly
/* CLEARING DMA HALF-TRANSFER AND TRANSFER-COMPLETE FLAGS IN ASSEMBLY */
    ldr     r0, =DMA1_IFCR
    movs    r1, #(1 << 2)       /* Bit 2 = CHTIF1 (Clear Half Transfer Flag) */
    str     r1, [r0]            /* Write 1 to clear HTIF1 in hardware! */

    movs    r1, #(1 << 1)       /* Bit 1 = CTCIF1 (Clear Transfer Complete Flag) */
    str     r1, [r0]            /* Write 1 to clear TCIF1 in hardware! */
```
...
### 1. L1 Data Cache Coherence Violations on High-Speed CPUs

On high-performance 32-bit microcontrollers equipped with an internal **Level 1 (L1) Data Cache** (such as ARM Cortex-M7 processors):

When the DMA engine writes incoming sensor samples directly into physical DRAM or SRAM memory across the system bus matrix, **it bypasses the CPU's private L1 Data Cache**!

```text
L1 DATA CACHE COHERENCE HAZARD IN BARE-METAL DMA

 1. DMA Engine writes fresh sample (42) DIRECTLY TO PHYSICAL SRAM MEMORY!
                               │
 2. CPU L1 Data Cache STILL HOLDS STALE OLD LINE (0) in SRAM cache!
                               │
                               ▼
 CPU Core executes LOAD [adc_buffer[0]] ──► Reads L1 Cache ──► READS STALE DATA 0!
```

#### The Physical Memory Failure:
1. The DMA engine writes sample `42` directly into physical SRAM address `0x2000_0000`.
2. The CPU core previously read address `0x2000_0000`, so its private L1 Data Cache holds a copy of `0x2000_0000` containing old value `0`.
3. When the `HT` interrupt fires and the CPU executes `LDR r0, [adc_buffer]`:
   * The CPU queries its L1 Data Cache.
   * **L1 Cache HIT!** The L1 cache returns the old, stale value `0` from SRAM cache!
   * The CPU completely misses the fresh sample `42` written by the DMA engine to physical RAM!

#### The Required Assembly Fix: L1 Cache Invalidation Before Reading
Inside the `HT` and `TC` interrupt handlers, before reading data out of the completed half-buffer, software **MUST invalidate the L1 Data Cache lines corresponding to that half-buffer**:

```assembly
/* INVALIDATING L1 DATA CACHE RANGE BEFORE READING DMA BUFFER */
    /* Inputs: r0 = Start Address of Half-Buffer, r1 = Length in Bytes */
    ldr     r2, =SCB_DCIMVAC    /* SCB Data Cache Invalidate by MVA Register */
cache_inval_loop:
    str     r0, [r2]            /* Invalidate 32-byte cache line at address r0 */
    add     r0, r0, #32         /* Advance to next 32-byte cache line */
    subs    r1, r1, #32
    bgt     cache_inval_loop
    dsb                         /* Data Synchronization Barrier */
    /* L1 Cache is now 100% invalidated! CPU will fetch fresh DMA data from RAM! */
```
...
## Solved Industrial Engineering Exercise: Quantitative DMA Bandwidth Offloading, Ping-Pong Timing Analysis, and Assembly Driver Synthesis

To consolidate your complete mastery of bare-metal DMA channel configuration, circular mode (`CIRC`), half-transfer (`HT`) and transfer-complete (`TC`) interrupts, ping-pong double buffer timing equations, and assembly driver synthesis, we will now walk through a complete, step-by-step industrial hardware engineering problem.
...
#### Step 1: Calculate Half-Buffer Fill Time ($T_{\text{fill\_half}}$)

Given:
* Sample Rate $f_{\text{sample}} = 200,000\text{ samples/sec}$.
* Half-Buffer Size $N_{\text{half}} = 200\text{ samples}$.

$$\text{Sample Period } T_{\text{sample}} = \frac{1}{200,000\text{ Hz}} = 5.00 \times 10^{-6}\text{ s} = \mathbf{5.00 \text{ Microseconds}}$$

Calculate time to fill 200 samples ($T_{\text{fill\_half}}$):

$$T_{\text{fill\_half}} = N_{\text{half}} \times T_{\text{sample}} = 200 \text{ samples} \times 5.00\ \mu\text{s/sample} = \mathbf{1,000.00 \text{ Microseconds}} = \mathbf{1.000 \text{ Millisecond}}$$

In CPU clock cycles ($T_{\text{clk}} = 0.3125\text{ ns}$):

$$\text{Cycles}_{\text{fill\_half}} = \frac{1,000,000.0\text{ ns}}{0.3125\text{ ns/cycle}} = \mathbf{3,200,000 \text{ CPU Clock Cycles}}$$

Each 200-sample half-buffer takes **$1.000\text{ millisecond}$ ($3,200,000\text{ CPU cycles}$)** to fill in the background!
...
#### Step 3: Calculate CPU Cycles Saved and Offloading Percentage

At $200,000\text{ samples/sec}$:

##### 1. Individual CPU Interrupt Overhead (Without DMA):
Each sample burns $40\text{ CPU cycles}$ (stacking + ISR + unstacking):

$$\text{Cycles}_{\text{interrupt\_no\_DMA}} = 200,000 \text{ samples/sec} \times 40 \text{ cycles/sample} = \mathbf{8,000,000 \text{ CPU Cycles / Sec}}$$

$$\text{Wasted CPU Power} = \frac{8,000,000}{3,200,000,000} \times 100\% = \mathbf{0.25\% \text{ of CPU Core Wasted}}$$

##### 2. Ping-Pong DMA Interrupt Overhead:
DMA generates only 2 interrupts per full buffer ($200\text{ interrupts/second}$ total at $1.0\text{ ms}$ per half-buffer!).

Each half-buffer interrupt burns $1,600\text{ processing cycles} + 24\text{ context cycles} = 1,624\text{ cycles}$:

$$\text{Cycles}_{\text{DMA\_PingPong}} = 1,000 \text{ interrupts/sec} \times 1,624 \text{ cycles/interrupt} = \mathbf{1,624,000 \text{ CPU Cycles / Sec}}$$

$$\text{Cycles Saved per Second} = 8,000,000 - 1,624,000 = \mathbf{6,376,000 \text{ CPU Clock Cycles Saved!}}$$

$$\text{Percentage Reduction in Interrupt Frequency} = \left( 1 - \frac{200 \text{ IRQs}}{200,000 \text{ IRQs}} \right) \times 100\% = \mathbf{99.900\% \text{ IRQ Reduction!}}$$

```text
PING-PONG DMA PERFORMANCE SAVINGS SUMMARY

 Metric                       │ Individual CPU Interrupts │ Ping-Pong DMA Double Buffer
──────────────────────────────┼───────────────────────────┼───────────────────────────────
 Interrupts per Second        │ 200,000 Interrupts/sec    │ 1,000 Interrupts/sec (500/half)
 CPU Cycles Burned per Sec    │ 8,000,000 Cycles/sec      │ 1,624,000 Cycles/sec
 Interrupt Frequency Reduction│ 0.0% (Baseline)           │ 99.900% IRQs Eliminated!
 Data Race Protection         │ ZERO (Read-While-Write)   │ 100% Protected (Half-Buffers)
```
...
1. **Half-Buffer Fill Time Verification**:
   * $f_{\text{sample}} = 200\text{ kSPS} \implies T_{\text{sample}} = 5.0\ \mu\text{s}$.
   * Half-buffer $= 200\text{ samples}$.
   * Fill time $= 200 \times 5.0\ \mu\text{s} = \mathbf{1,000.0 \ \mu\text{s}} = \mathbf{1.000 \text{ ms}}$. Math verified $100\%$!

2. **Overrun Safety Margin Verification**:
   * $T_{\text{proc}} = 0.50\ \mu\text{s} \ll T_{\text{fill\_half}} = 1,000.0\ \mu\text{s}$.
   * Safety margin $= 2,000\times$, guaranteeing that the CPU finishes processing Half-Buffer 0 $999.5\ \mu\text{s}$ before the DMA engine wraps around to overwrite it!

3. **Circular Reload Invariant Verification**:
   * `CIRC = 1` in `DMA1_CCR1`.
   * When `CNDTR` reaches $0$, hardware automatically reloads `CNDTR <= 400` and resets `CMAR <= &adc_buffer[0]` in $0\text{ clock cycles}$, providing unbroken $100\%$ zero-copy data streaming.

4. **W1C Flag Clearing Verification**:
   * `DMA1_IFCR` bit 2 (`CHTIF1`) and bit 1 (`CTCIF1`) were written with $1$.
   * Cleared status flags in hardware before exception return, preventing infinite ISR trap loops!

All DMA channel register mappings, circular mode reload algorithms, half-buffer index offsets, `HT`/`TC` interrupt state transitions, and assembly driver implementations evaluate with 100% mathematical, physical, and logical precision.
